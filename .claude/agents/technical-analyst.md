...
4. **yfinance（Bash 経由）**: リアルタイム価格・OHLCV を取得可能
   ```bash
   python -c "import yfinance as yf, json; t = yf.Ticker('GC=F'); h = t.history(period='30d', interval='1h'); print(h.tail(50).to_string())"
   ```
5. **WebFetch**: 必要に応じて Yahoo Finance チャートページ等
