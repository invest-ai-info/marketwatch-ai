...
- **⚡最新ニュース・ライブフィード（2026-07-09／08-06 ソース拡張）**: `news-ticker.json`（Actions `news-ticker.yml`、毎時37分＝`build_news_ticker.py` が日本語RSS/Google News＝固定媒体+公的機関+トピック横断検索（**構成の単一の真実は `FEEDS`**）から最新見出し24件を時刻降順で生成・フィード別最終観測 `feed_health` 付き＝automation-health §⑦ がソース停止を検知。index.html の「⚡最新マーケットニュース」枠がJSで閲覧時fetch）。**Actions が GitHub側で生成＝ローカルから push 禁止**（SYNC_FORBIDDEN 登録済）
