...
- **デイリーニュース記事 下書き（2026-06-15 追加）**: `drafts/news/draft-news-<YYYY-MM-DD>.html` / `drafts/news/NEWS_LEDGER.md`（**ローカルのスケジュールタスク `news-daily`**＝Claude Codeアプリの予約タスク、毎日17:30 JST。アプリが開いていれば実行、閉じていれば次回起動時。⚠️cloud routine ではないので**アプリ起動時にのみ走る**＝24/7にしたいなら claude.ai/code/routines に同SOPで移植）。手順書 `drafts/NEWS_DAILY_GUIDE.md` に従い、その日の最重要マーケットニュースを「話題性×影響×付加価値」でスコア→**1本だけ**中立整理の下書きを生成。**薄い日は見送り**（合計9/15未満等）。対象＝マクロ＋個別銘柄の両方（個別銘柄は「買い/売り」と読ませないフラット整理厳守）。**出力はローカル保存のみ＝push しない・本番ファイルに触れない・自動公開しない**（公開は人間が下書きを見て `mw publish --category "今日のニュース"` で。signal-lab と違い決定論的検証オラクルが無いので当面ワンクリック公開、安定したら自動公開を検討）。手順書 `NEWS_DAILY_GUIDE.md` は人が編集＝**SYNC_FILES入り**、`drafts/news/*` 出力は SYNC外（ローカル生成）
