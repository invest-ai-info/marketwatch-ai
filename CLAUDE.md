...
- **シグナル研究日誌・日次研究会（2026-06-11 追加）**: `signal-lab-ledger.md`（研究台帳）/ `drafts/draft-signal-lab-*.html` / `drafts/labnotes/lab-*-analysis.md`（routine `signal-lab-daily` `trig_01V4A37Xow1vx2QAAvYwzR57`、毎朝 06:10 JST＝NY引け後。投資3視点〈technical/fundamental/risk-manager ペルソナ〉で勝率改善仮説を**1日1本だけ**signals-logで反実仮想検証し、研究日誌の**下書き**＋検証生ログ（labnotes＝スクリプト全文と生出力、人間の数字照合用）＋台帳を生成・コミット）。**routine が GitHub 側で生成するため SYNC_FILES に入れない**（ローカルから push 禁止）。公開は必ず人間レビュー（labnotes との数字照合・SVG実機確認・compliance-reviewer(Opus)監査）を経て `guide-signal-lab-NNN.html` として行う（無人公開しない）。エンジン・発火条件には絶対に触れない設計（検証は常に「もしも」計算のみ）。記事番号は研究日誌シリーズの続番（#3〜）、台帳が次番号を管理
