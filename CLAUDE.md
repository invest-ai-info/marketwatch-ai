...
| Agent | 配置 | 役割 | モデル |
|---|---|---|---|
| **technical-analyst** | `.claude/agents/technical-analyst.md` | チャート / シグナル / ATR / RSI / MACD / BB / MA / 出来高の評価 | Sonnet |
| **fundamental-analyst** | `.claude/agents/fundamental-analyst.md` | 経済指標 / 決算 / 地政学 / 政治発言 / 金融政策 / マクロの評価 | Sonnet |
| **risk-manager** ⭐ | `.claude/agents/risk-manager.md` | 統合判断・規律遵守の門番。SL/TP/ロット算出、金曜大引け / 環境警戒 / 反転検知のチェック、過信防止 | Opus |
...
ユーザー「明日 NKD=F でロングを検討してる」
   ↓
メイン Claude が並列で 2 人に依頼（同一メッセージ内で Agent ツール 2 つ並列起動）
   ├─ technical-analyst: 4H/1H チャート、シグナル、ATR、サポレジ、マルチ TF 整合性
   └─ fundamental-analyst: 来週の経済指標、地政学、円相場、関連政治発言
   ↓
両方の結果を risk-manager に渡す（次のメッセージで Agent 呼び出し、prompt に 2 人の出力を埋め込み）
   ├─ テクニカル × ファンダの整合性
   ├─ SL/TP/ロット算出（ATR ベース、SL=±1.5×ATR / TP1=±2.0×ATR / TP2=±3.0×ATR）
   └─ ユーザールール照合（金曜大引け、環境警戒 D 見送り、反転検知 12h 以内見送り、B2 HIGH のみ）
   ↓
メイン Claude が「🟢 ✅ 条件成立」「🟡 ⚠️ グレー」「🔴 ❌ 見送り推奨」+ 理由を提示
   ↓
ユーザーが最終判断（AI は意見、決めるのはあなた）
```

### 自動委譲のトリガー

各 agent の `description` フィールドに英語キーワード + 日本語キーワードを書いている。例:
- 「チャートはどう？」「テクニカル的に」「シグナル発火」→ technical-analyst
- 「FOMC 控えてる」「決算は」「ファンダ的に」「地政学」→ fundamental-analyst
- 「エントリーしていい？」「SL どこ置く」「リスク的にどう」→ risk-manager

明示呼び出しの例: 「risk-manager に聞いて、今 GC=F に入っていい？」

### 設計の原則

1. **これは投資助言ではなく参考分析** — 各 agent は出力に必ず明記。最終判断はユーザー本人
2. **N=6 戦 6 勝の罠に注意** — 直近実績は小サンプル。risk-manager が過信を抑える役を担う
3. **規律の門番は妥協しない** — 金曜大引け・環境警戒 D・反転検知ありは無条件で見送り推奨
4. **サイト公開しない** — agent の出力はユーザー個人向け。サイトに転載すると「無登録投資助言業」リスクに直結
5. **連携はテキスト経由** — メイン Claude が agent 間でテキストを橋渡し（subagent 間の直接通信機構は無い）
...
- `memory/01_profile.md` — 投資家プロファイル（固定情報、年単位で更新）
- `memory/02_evolution.md` — 投資スタイル進化記録（月初の Self-Review で更新）
- `memory/03_initiatives.md` — 進行中の検証・打ち手（週次で更新）

**🆕 セッション開始時の必読順序**:
1. `CLAUDE.md`（このファイル、全体像）
2. `SESSION_HANDOFF.md`（直近進捗）
3. `memory/01_profile.md` + `memory/02_evolution.md` + `memory/03_initiatives.md`（ユーザー固有・進化情報）

**月初（1-3 日）のセッション開始時は必ず**:
- マンスリーレポートを読み、`02_evolution.md` の方針更新を Claude が提案する
