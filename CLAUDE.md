...
- **シグナル研究日誌・日次研究会＋自動公開（2026-06-11 / 2026-06-13 自動公開化）**: `signal-lab-ledger.md`（研究台帳）/ `drafts/draft-signal-lab-*.html` / `drafts/labnotes/lab-*-analysis.md` / `drafts/labnotes/lab-*-claims.json`（routine `signal-lab-daily` `trig_01V4A37Xow1vx2QAAvYwzR57`、毎朝 06:10 JST＝NY引け後。投資3視点で勝率改善仮説を**1日1本だけ**signals-logで反実仮想検証し、研究日誌の下書き＋labnotes＋**claims.json**＋台帳を生成・コミット）。**routine が GitHub 側で生成するため SYNC_FILES に入れない**（ローカルから push 禁止）。**🆕 2026-06-13＝オーナー承認のもと「全自動公開（白のみ）」に移行**。自動公開ゲート＝①`signal_lab_verify.py`（SYNC入りの固定コード。claims.json の全 k/n を signals-log から独立再計算して突合＋『30秒まとめ』要約ボックスの%完全性＋SVGはみ出し）が exit 0、かつ ②Opusコンプラ監査が🟢白＋グレー指摘ゼロ、の**両方が緑のときだけ自動で `finalize_signal_lab.py`→`publish_article.py`→push**。**1つでも非緑（赤/グレー/未検証/opus不可）なら公開せず REVIEW.md に「🚩要人間レビュー」で記録してエスカレ**（疑わしきは公開しない）。⚠️publish の `--category` はバッジ文字「AIシグナル研究日誌」＝**絵文字なし**（🧪は --emoji で別途）。エンジン・発火条件には絶対に触れない（検証は常に「もしも」計算）。記事番号は台帳が管理。**人間の役割＝エスカレされた回のレビューのみ**（緑の回は自動公開済み、事後スポットチェック可）
