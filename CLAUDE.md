...
- `sitemap.xml`, `robots.txt`, 個別 guide-*.html
...
5. `generate_market_news.py` の「📰 更新履歴」に新エントリ追加
   ...
   - ⚠️ **週次戦略記事（`guide-weekly-*.html`）は手動追記しない**：2026-05-31 以降、`build_weekly_history_entry()` が最新の guide-weekly を自動検出して更新履歴の先頭に自動登録する（同時に `build_weekly_strategy_banner()` が index.html 上部にバナーも自動生成）。手で足すと二重表示になる
