...
- **研究ラボの出力（2026-09-26）**: `exit-lab.*`／`exit-wall-lab.*`／`stop-lab.*`／`regime-lab.*`／`signal-env-profile*`（Actions が生成・**前向きの判定の履歴を持つ**＝古い版で上書きすると積み上げた判定が消える）。SYNC_FORBIDDEN 登録済み
...
| **`publish_article.py`** | 記事公開の②〜⑤を1コマンド・冪等（`mw publish` が内部で使用）。🆕**上書きゲート**＝git の HEAD 版と `datePublished` が違えば「公開済み記事を別記事で上書き」と判断して中止（`--allow-overwrite` で解除）。2026-09-06 に #089 が消えた事故の再発防止。🆕**カテゴリゲート**＝入れる欄が決まらなければ書き込む前に中止（旧版は記事一覧の先頭＝計算ツール欄へ黙って入れ、47枚が紛れた。2026-09-26）。点検側は `check_site_consistency` §8 が data-category の欄への紛れ込みを error |
