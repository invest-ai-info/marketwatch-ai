...
- **シリーズ自動公開レーン（1日1本・キュー駆動。2026-08-31 に一覧化）**: 手順書＋題材キューは **SYNC入り（人が編集）**、台帳と記事は **GitHub側生成**。
  - `proverb-daily-auto` `trig_01P8Jjut79fHWoiALk4mfuJQ`（毎日 10:12 JST）＝投資格言。手順書 `drafts/PROVERB_GUIDE.md`／台帳 `drafts/proverb/PROVERB_LEDGER.md`／記事 `guide-proverb-*.html`。⚠️ **44本で和の相場格言をほぼ汲み尽くした**（8/19・8/31 と2度枯渇）。残り4本を出し切ったら、44本の総集ハブを作って**完結させ、このルーティンを止める**判断をオーナーに諮ること
  ...
  - 🚨 **レーンを増やしたら、必ず `check_automation_health.py` の `QUEUE_LANES` に登録すること。**§⑤は「キュー枯渇による静かな停止」を捕まえる番人なのに、2026-08-31 まで **autodraft の1本しか見ておらず**、投資格言5日連続・投資詐欺7日連続の枯渇を誰も知らないまま放置していた（ルーティン自身は毎日きちんと台帳へ「🚩補充依頼」を書いていた）
...
| **`inject_ads.py`** | A8.net のアフィリエイト広告を、商品ごとに**関連の高い記事だけ**へ冪等注入（`ADS` に商品ブロック／`PAGES` に対象ページ）。**「広告」ラベル必須**＝景表法のステマ規制（2023-10-01 施行）。⚠️ **本文に推奨文・煽り文言を足さない**＝バナーのみ（金商法の誇大広告／投資勧誘に寄せない）。既存枠＝DMM株・DMM CFD、🆕2026-09-10 に **FX**（FXが主題の常設記事7本）を追加 |
