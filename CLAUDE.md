...
- **記事下書き（2026-06-06 追加）**: `drafts/draft-*.html` / `drafts/REVIEW.md`（routine `autodraft-article` `trig_01VpreEMybEJCmFiU5TS7Vet`、毎日 05:30 JST が心理＆リスク管理シリーズの**下書き**を生成・コミット）。**routine が GitHub 側で生成するため SYNC_FILES に入れない**（ローカルから push 禁止）。下書きは `noindex,nofollow`＋robots.txt で `/drafts/` Disallow。**例外＝`drafts/AUTODRAFT_GUIDE.md`／`AUTOPUBLISH_GUIDE.md`／`BOOKWATCH_GUIDE.md`（手順書＋topicキュー）は人間が編集するので SYNC_FILES 入り**。**🆕 2026-07-05＝オーナー承認で公開も完全無人化**：routine `autodraft-publish`（毎日 08:40 JST）が最古の未公開下書きを仕上げ→**決定論ゲート `check_guide_draft.py`（固定ゲート・編集禁止＝noindex消し忘れ/kinsho-v1/ナビ10/TODO残り/売買推奨ハードNG/SVGはみ出し・重なり）exit 0 → Opusコンプラ+品質(QUALITY_RUBRIC)白（🟡軽微はOpus修正→再ゲート→独立Opus白確認）→ publish_article.py で公開＋PUSH-MAIN**。🔴黒/要協議/赤は公開せず REVIEW.md に🚩エスカレ。手順書=`drafts/AUTOPUBLISH_GUIDE.md`
- **研究アイデア受信箱（2026-07-07 追加）**: `drafts/idea-inbox.md`（routine `idea-scout-weekly`、毎週日曜 14:00 JST。WebSearchで検証可能な新手法・論文を最大3件、事前登録形式で**追記のみ**。重複回避は `drafts/idea-tested-slugs.txt`＝SYNC入りと照合）。**routine が GitHub 側で追記するため SYNC_FILES に入れない**（check_site_consistency 登録済・照合はbasename）。ローカル進化ループ（`mw evolve`／`research/DOCTRINE.md`＝ローカル専用）の①INTAKE
- **投資本 新刊ウォッチ（2026-07-05 追加）**: `guide-new-books.html`（routine `book-watch-weekly`、毎週土曜 11:00 JST。WebSearchで直近30日の投資系新刊を探し2ソース照合→中立紹介を最新40冊まで積み上げ。初回はページ新規作成+guides.htmlカード追加）。**routine が GitHub 側で更新するため SYNC_FILES に入れない**（ローカルから push 禁止・check_site_consistency の SYNC_FORBIDDEN 登録済）。手順書=`drafts/BOOKWATCH_GUIDE.md`（SYNC入り）
...
- **シグナル研究日誌・日次研究会＋自動公開（2026-06-11 / 2026-06-13 自動公開化）**: `signal-lab-ledger.md`（研究台帳）/ `drafts/draft-signal-lab-*.html` / `drafts/labnotes/lab-*-analysis.md` / `drafts/labnotes/lab-*-claims.json`（routine `signal-lab-daily` `trig_01V4A37Xow1vx2QAAvYwzR57`、毎朝 06:10 JST＝NY引け後。投資3視点で勝率改善仮説を**1日1本だけ**signals-logで反実仮想検証し、研究日誌の下書き＋labnotes＋**claims.json**＋台帳を生成・コミット）。**routine が GitHub 側で生成するため SYNC_FILES に入れない**（ローカルから push 禁止）。**🆕 2026-06-13＝オーナー承認のもと「全自動公開」に移行**。自動公開ゲート＝①`signal_lab_verify.py`（**SYNC入りの固定コード＝独立オラクル。routine/エージェントは編集禁止・実行前に `git checkout` で確定版に戻す**。claims.json の全 k/n を signals-log から独立再計算して突合＝捏造は通らない＋『30秒まとめ』%完全性＋SVGはみ出し＋**未対応フィルタキーは即赤**）が exit 0、②Opusコンプラ監査。**公開方針＝検証緑かつOpusが「黒でなく要協議でもない」なら自動公開。🟡グレー(軽微修正でOK)は**コンプラ担当Opus自身が軽微修正(表現軟化・免責追加のみ・数値/SVG/要約不変)を適用→数値再検証→別の独立Opusが白を確認**してから公開（修正者の自己承認を避ける）。🔴黒・🟡要協議・検証赤・未対応次元・opus不可のときだけ公開せず REVIEW.md に「🚩要人間レビュー」でエスカレ**。filterキー＝ticker/group/direction/trend/tf/signal/reversal_long/blocked/tier（新次元が必要な仮説はverify.pyを編集せずエスカレ＝人間がゲートを拡張）。⚠️publish の `--category` はバッジ文字「AIシグナル研究日誌」＝**絵文字なし**（🧪は --emoji で別途）。エンジン・発火条件には絶対に触れない。記事番号は台帳が管理。**人間の役割＝エスカレされた回のレビューのみ**
- **X(SNS)投稿ドラフト（2026-06-13 追加）**: `drafts/sns/<YYYY-MM-DD>.md`（routine `sns-post-daily` `trig_01VkDY4djA8WAAZavhgu3j4M`、毎日 07:00 と 19:00 JST＝cron `0 10,22 * * *` UTC の2回。JST時刻を見て朝/晩を出し分け、@rx009898 が手動コピペ投稿するX投稿文を生成・コミット）。価格は Yahoo chart API 直叩き、地合いは fundamental-context.json、イベントは economic-events.json から。X重み付き文字数280以内・免責短縮版必須。**routine が GitHub 側で生成するため SYNC_FILES に入れない**（ローカルから push 禁止）。**⚠️ X API による自動投稿はしない**（無料枠は2026-02に廃止＝従量課金のみ。コピペ専用の下書き）。`/drafts/` は robots.txt で Disallow 済
- **デイリーニュース記事 完全自動公開（2026-06-15 下書き運用→2026-06-17 クラウド自動公開化）**: クラウドルーティン **`news-daily-auto`（`trig_01WZ9maArFYwaxyq99KJLJBi`、毎日 17:40 JST＝cron `40 8 * * *` UTC）**が、手順書 `drafts/NEWS_DAILY_GUIDE.md` に従いその日の最重要マーケットニュースを「話題性×影響×付加価値」でスコア→**1本だけ**中立整理→**Opus自動公開ゲート（signal-lab方式：🟢白は `publish_article.py --category "今日のニュース"` で自動公開＋git push／🟡軽微はOpus自己修正→別Opus独立確認→公開／🔴黒・要協議・事実未確定はエスカレ＋免責三層/禁止語/銘柄推奨無し/出典2系統の決定論チェック）**で公開。公開物は `guide-news-<YYYY-MM-DD>-<slug>.html`（noindexでない通常記事）＋`guides.html`「今日のニュース」カード＋`drafts/news/NEWS_LEDGER.md`（判定履歴・GitHub側生成＝SYNC禁忌）。**薄い日は見送り**（合計9/15未満等）。対象＝マクロ＋個別銘柄（個別は「買い/売り」と読ませないフラット整理厳守）。⚠️**旧ローカル予約タスク `news-daily` は無効化済（アプリ起動依存＋無人実行がツール権限待ちで停止する不具合があったため）。二重公開防止でクラウドに一本化**。⚠️発火エンジン・signal_lab系・固定オラクルには触れない（実行のみ）。手順書 `NEWS_DAILY_GUIDE.md` は人が編集＝**SYNC_FILES入り**、`drafts/news/*` 出力は SYNC外。クラウドルーティンは `RemoteTrigger`（list/get/run/create/update）で管理可

- **⚡最新ニュース・ライブフィード（2026-07-09 追加）**: `news-ticker.json`（GitHub Actions `news-ticker.yml`、毎時37分。`build_news_ticker.py` が日本語RSS/Google Newsから最新見出し24件を時刻降順で生成・コミット。index.html の「⚡最新マーケットニュース」枠がJSで閲覧時にfetch）。**Actions が GitHub 側で生成・コミットするため、ローカルから push 禁止**（SYNC_FILES に入れない・check_site_consistency の SYNC_FORBIDDEN 登録済）

- **日本株ランキング（2026-06-20 追加）**: `jp-rankings.json`（GitHub Actions `jp-rankings.yml`、夕 16:40/17:10 JST＝クローズ後。※朝実行はcron遅延で場中更新になるため廃止。`build_jp_rankings.py` が Yahoo価格で値上がり/値下がりトップ20を生成・コミット。`generate_market_news.py` の `build_jp_rankings_section` が hot-assets.html 最上段に描画）。**Actions が GitHub 側で生成・コミットするため、ローカルから push 禁止**（SYNC_FILES に入れない）。※`jp-stock-info.json`（赤字黒字/名前/業種の静的情報）と `build_jp_rankings.py` は SYNC_FILES 入り＝ローカルで `make_jp_stock_info.py` により四半期更新して push
