...
コマンド:
  ...
  python mw.py discipline                  規律ギャップ・レポート（実取引×ルール×JPカード履歴の決定論突合。
                                           週次 /loop レシピ＝「mw discipline→逸脱トップ3診断→STOP」）
  python mw.py evolve                      進化ループの状態（DOCTRINE突合＋仮説キュー＋トラッカー鮮度＝
                                           _doctrine_check.py --agenda。決定論・トークン0）
  python mw.py screen [フィルタ...]         日本株 個人用スクリーナー（検証済み知見で全銘柄採点＝
                                           危険スコア/赤字/グレアム/防御列/TT3。--max-danger 0 --min-graham 4 等）
  python mw.py screen --policy [--refresh] 骨太2026 政策テーマ銘柄の「出遅れ×割安×黒字」選別（17分野・
                                           _jp_policy_value.py。--refresh=IRBank財務を四半期再取得）
  python mw.py size --balance N --pos ...  ポジションサイズ計算機（1%既定・相関合算2%・全体5%目安の参考計算。
                                           例: mw size --balance 3000000 --pos "GC=F,long,2650,2635"）
...
def cmd_discipline(argv):
    """規律ギャップ: 実取引×MY_TRADING_RULES×JPカード履歴を決定論突合（読取専用・トークン0）。"""
    return _run(["_trade_discipline_check.py"] + argv)


def cmd_evolve(argv):
    """進化ループ: DOCTRINE.mdの数値突合＋仮説キュー状態＋トラッカー鮮度（読取専用・トークン0）。"""
    if not os.path.exists(os.path.join(SD, "_doctrine_check.py")):
        # mw.py は SYNC 対象＝クラウド側にも存在するが、_doctrine_check.py はローカル専用。
...
def cmd_screen(argv):
    """個人用スクリーナー: 検証済み知見（危険/赤字/グレアム/防御/TT3）で全銘柄採点（ローカル専用）。
    --policy = 骨太2026 政策テーマ銘柄の「出遅れ×割安×黒字」選別（_jp_policy_value.py・--refresh 併用可）。"""
    if "--policy" in argv:
        rest = [a for a in argv if a != "--policy"]
        if not os.path.exists(os.path.join(SD, "_jp_policy_value.py")):
            print("screen --policy はローカル専用（_jp_policy_value.py はリモートに存在しないのが正常＝再作成禁止）")
            return 0
        return _run(["_jp_policy_value.py"] + rest)
    if not os.path.exists(os.path.join(SD, "_jp_screener.py")):
        print("screen はローカル専用（_jp_screener.py はリモートに存在しないのが正常＝再作成禁止）")
        return 0
    return _run(["_jp_screener.py"] + argv)


def cmd_size(argv):
    """ポジションサイズ計算機: 検証済みリスク枠組み（1%既定・相関合算2%・全体5%目安）の参考計算（ローカル専用）。"""
    if not os.path.exists(os.path.join(SD, "_position_size.py")):
        print("size はローカル専用（_position_size.py はリモートに存在しないのが正常＝再作成禁止）")
        return 0
    return _run(["_position_size.py"] + argv)
...
CMDS = {
    ...
    "declutter": cmd_declutter, "discipline": cmd_discipline, "evolve": cmd_evolve,
    "screen": cmd_screen, "size": cmd_size,
}
