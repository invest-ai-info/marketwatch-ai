...
2026-09-20 autodraft: スキップ（対象なし：topicキュー全63件が下書き済みまたは公開済み。新規下書きなし。キュー補充が必要）
2026-09-19 bookwatch: 『AIに9割まかせる ほぼほったらかし株式投資 株価が3.53倍、利益253万円に爆増！』（投資家Go/KADOKAWA/2026-09-11）を除外。副題に具体的利益額（3.53倍・253万円）の煽り表現あり、著者が運営する有料投資スクール（The Gavel・AIA）へのファネルとなる教材商法的要素が懸念されるため掲載せず。
...
2026-09-17 signal-lab: 🚨 #097 は🚩エスカレ中なのに finalize 済みの `guide-signal-lab-097.html` がルートに残っており、**sitemap に載り #096「次」/#099「前」から読めてしまう状態だった**（9/13〜9/17）。応急処置として ①ルート版に noindex を付与 ②sitemap から削除 ③前後ナビを #096↔#099 に貼り替え ④#100 本文のリンクを解除 を実施。**ファイル自体の削除はオーナー承認待ち**。再発防止＝`check_site_consistency.py` が「カード無し＆noindex無し」を error にする／`apply_series_nav.py` は noindex を並びに入れない。　**【2026-09-17 追記・現状】[解決済み]（削除で決着＝公開したのではない）／ルートからは削除済み・下書きのみ保持**＝オーナー承認を得て `guide-signal-lab-097.html` をルートから削除（コミット `9923b5c`、709行削除）。実測の現状＝ルートに無し／`sitemap.xml` 不掲載／`guides.html` カード無し／公開HTMLからの残リンク0件／前後ナビは #096→#099→#100 で連続。本文は `drafts/draft-signal-lab-097.html` に noindex 付きで保持（`drafts/labnotes/lab-097-analysis.md` / `lab-097-claims.json` も保持）。🚨 **復元しないこと**＝コンプラ未承認の記事が再び公開状態に戻る。承認された場合のみ `finalize_signal_lab.py` + `publish_article.py` の正規手順で公開する。**台帳（`signal-lab-ledger.md` の `## 2026-09-13 #097`）側も同じ内容に更新済み。🔖 **この `[解決済み]` は番人（automation-health §⑧）を黙らせるための印であって、コンプラ審査を通ったという意味ではない。**§⑧ は「成果物の記事が公開されたか」で解決を判定するため、削除で閉じた案件は印が無いと永久に滞留として鳴り続ける（オーナー判断 2026-09-23）。承認して公開する場合は、この印ごと書き直すこと。**
...
- **テーマ**: 上昇中の逆張り買い（trend=上昇×reversalL）— 降格警戒ストライク1・RSI型62% vs BB型42%の分裂
- **verify**: ✅ GREEN（6/6 claims緑・要約未検証0件・SVG警告0件）＝**k/n と % は正しい**
- **Opusコンプラ**: 🟡 グレー4件→軽微修正適用（下記）。金商法・景表法の観点では**黒なし**
- **Opus品質（QUALITY_RUBRIC ③）**: ❌ **勝率95%CI列が独立再計算と一致しない**（数値訂正は監査側の権限外＝人間レビュー）

### 🚩 品質❌: 表の「95%CI（勝率）」5行が signals-log から再現できない（数値訂正・要人間レビュー）

`signal_lab_verify.py` は k/n と % を検証するが **CI 境界は検証対象外**。監査で独立に再計算したところ、記事掲載の CI はウィルソン法・正規近似・Clopper-Pearson のいずれとも一致しなかった（＝生成時の推定値の可能性が高い）。

| 行 | k/n | 記事掲載CI | ウィルソン法（verify出力） | 正規近似 |
|---|---|---|---|---|
| IS | 54/101 | [42.0%, 64.7%] | **[43.8%, 62.9%]** | [43.7%, 63.2%] |
| FWD全体 | 150/318 | [41.2%, 53.3%] | **[41.7%, 52.7%]** | [41.7%, 52.7%] |
| FWD RSI型 | 48/77 | [48.8%, 74.7%] | **[51.2%, 72.3%]** | [51.5%, 73.2%] |
| FWD BB型 | 102/241 | [35.5%, 49.4%] | **[36.3%, 48.6%]** | [36.1%, 48.6%] |
| FWD 日足 | 16/23 | [42.0%, 91.6%] | **[49.1%, 84.4%]** | [50.8%, 88.4%] |

- 方向はすべて**実際より広い（＝保守的）**ため、優位性を過大に見せる誤りではない。景表法・金商法上の直接の違反リスクは低い。
- ただし本シリーズの売りは「独立オラクルで再現できること」なので、再現不能な統計値の掲載は信頼性の問題。
- 台帳 `signal-lab-ledger.md` #103 の記載も混在（IS・FWD全体は記事と一致／RSI型・BB型・日足はウィルソン値と一致）＝生成側で計算経路が揺れている疑い。
- **修正案**: 記事の CI 列を上表の「ウィルソン法（verify出力）」で置換し、`signal_lab_verify.py` を再実行 → 緑なら公開可。%・k/n・E(R)・RCI は変更不要。

### 🚩 参考（軽微・ブロッカーではない）

- トラッカー表は `150/315・勝率48%`、本文は `150/318・47.2%`＝ n が 3 件食い違う（基準日が異なるため）。監査側で「上表は基準日時点の集計のため本文と件数がわずかに異なる」旨の注記を1文追加済み。
- トラッカー表の**基準日が 2026-09-19（未来日）**。公開日と整合するか要確認。

### コンプラ側で適用した軽微修正（表現のみ・数値/SVG/構成は不変）

1. 30秒まとめ③「損益分岐43%を**下回り続けている**」→「43%**前後で伸び悩んでいる**」（本文の月別 9月45.5% と矛盾していたため）
2. 「20ppの大きな差」に **pp（パーセントポイント）の初出説明**を追加（ルーブリック②）
3. RSI型の「引き続き**有望な候補です**」→「現時点までのデータでは比較的良好な結果が続いています（今後も同様に推移するとは限りません）」（将来の優位性を示唆する推奨的トーンの軟化）
4. BB型「CI[35.5%, 49.4%]**で損益分岐43%を下回り**」→「点推定は43%をわずかに下回り、CI は43%を挟んでいます」＋「優位性が低い」→「優位性が高いとは言い切れない」
5. トラッカー節に件数差の注記を1文追加
...
- **ゲート状態**: 🔴 エスカレ中（B-1/Y-D/Y-B 要人間レビュー）— 決定論✅・3rd Opus🟢達成済み。自律公開不可。
- **verify**: ✅ GREEN（9/9 claims全緑・SVG警告0件）
- **Opusコンプラ**: 1st→🔴 B-1 + 🟡 Y-1〜Y-7（Y-1〜Y-7自己修正済み）/ 2nd→🟡 Y-A/Y-C/Y-B/Y-D（Y-A/Y-C自己修正済み・Y-B/Y-D追加エスカレ）/ 3rd（独立）→🟢 白（Y-A/Y-C確認・新規🔴なし）
- **生成ファイル**: `drafts/draft-signal-lab-097.html`・`drafts/labnotes/lab-097-claims.json`・`drafts/labnotes/lab-097-analysis.md`

### 🚩 Y-D: グループ表（§⑤）の btc/metal/oil 行が未登録（要人間レビュー、B-1と同型）

**問題**: L477-485 グループ表に btc（N=23, k=13, 56.5%）・metal（N=21, k=11, 52.4%）・oil（N=14, k=8, 57.1%）の3行があるが、
`lab-097-claims.json` に未登録 = `signal_lab_verify.py` の検証対象外のまま、CI・E(R)付きで検証済み行と同じ体裁で並んでいる。
また6グループ合計299件 ≠ 全体301件（差2件 = 拡張指数ユニバース ^SOX/^GDAXI）が非開示。

**修正選択肢**（人間が判断）:
1. btc/metal/oil の3件を claims.json に追加し signal_lab_verify.py 再実行 → グループ合計の注記追加
2. btc/metal/oil 行を「小標本・参考値・検証範囲外」と明示するか、表から除外

### 🚩 Y-B: §⑦（考察・まとめ）に N=81 の typo 残存

**問題**: 記事本文に「指数グループ（N=**81**）のさらなる解析」という記述があるが、
L484の表・30秒まとめ・独立再計算はいずれも N=**80**（expired→closed補正後の正しい値）。
本記事の他箇所はすべて N=80 で統一されており、これのみ旧値が残存。

**修正**: N=81 → N=80（1桁の文字置換）。人間レビュー時に B-1/Y-D と同時に修正。

### 🚩 B-1: §⑥比較表の n 値に expired 混入（要人間レビュー）

**問題**: 記事§⑥（コンテキスト比較表）と30秒まとめの「参考：上昇×順張り買い」行が、
`closed() = {tp1,tp2,sl}` ではなく expired も含む n で集計されています。

| 記事の記載 | オラクル独立再計算 | 差 |
|---|---|---|
| 上昇×順張り買い N=633・38.9%・CI[35.1〜42.7] | N=622・k=246・39.5% | n が11件過大 |
| 上昇×ショート N=370・39.2% | N=358・k=145・40.5% | n が12件過大 |
| 全体ロング（参考）N=2,612・43.6% | N=2,576・k=1,138・44.2% | n が36件過大 |

**影響**: 比較相手の勝率が実際より低く見え、「逆張りが優位」という主張が過大に見える方向にバイアス。
正しい差は 47.8% − 39.5% = **8.3pp**（記事は8.9pp）。

**修正選択肢**（人間が判断）:
1. 3件を claims.json に追加し n を修正 → signal_lab_verify.py 再実行 → 数値更新・CI重複の注記追加
2. §⑥比較表と30秒まとめの参考行を削除し、検証済み9件の範囲のみで構成

**注記**: 検証済み9件（主クレーム）はすべて正確。B-1 は「未登録の参考比較行」のみの問題。
...
- **ゲート結果**:
  - 決定論(`check_guide_draft.py`) EXIT=0 🟢
  - Opusコンプラ: 🟢白（金商法・景表法・AdSense すべてクリア）
  - Opus品質ルーブリック: ①✅ ②⚠️軽微 ③❌ブロッカー2件 ④✅ ⑤✅
  - 総合: 🔴黒（エスカレ）— コンプラ起因ではなく品質起因。下書きは削除しない。
- **ブロッカー内容（テキスト置換4箇所のみ・座標/図形/数値は不変）**:
  - **(A)** `drafts/draft-bid-ask-spread.html` L466-467の誤記（Bid/Askを「市場の中間」と呼んでいるが互いに矛盾）:
    - `Bid（市場の中間）` → `仲値（Bid と Ask のまん中）`
    - `Ask（市場の中間）` → `仲値（Bid と Ask のまん中）`
  - **(B)** 同ファイルのSVG2（section-2）内の軸ラベル逆転（エントリー点はy=150の線上にあるのに「損益ゼロ」ラベルが付き、y=110の線に「エントリー価格（Ask）」が付いている）:
    - `エントリー価格（Ask）` → `損益ゼロ（Bid がエントリー価格に到達）`
    - `損益ゼロ` → `エントリー直後（−スプレッド）`
  - **(C)** ②軽微: L310の「板」に初出説明を追加（例：「板（＝売り買いの注文が並んだ気配値の一覧）」）
- **人間の作業手順**: (A)(B)(C)を `drafts/draft-bid-ask-spread.html` に適用 → check_guide_draft.py 緑確認 → Opus監査（白確認）→ publish_article.py → push。仕上げ編集（noindex除去・日付・nav・favicon・ブランドカラー）は本ルーティンが実施済みのため、下書き自体には残っている。
- **注**: 下書き削除なし（法務リスクによるエスカレではないため）。次回autopublish実行時に本件は既にエスカレ扱いとして guides.html カード確認でスキップされる（カード未登録）。
...
- **対象**: `drafts/draft-shareholder-benefits.html` → `guide-shareholder-benefits.html`
- **決定論ゲート**: ✅ GREEN（EXIT=0）
- **コンプラ（Opus）**: 🟢白（🟡軽微3件は修正済み）
- **品質ルーブリック（Opus）**: ❌ 観点③ — **L348 に事実誤認あり**
- **エスカレ理由**:
  - `guide-shareholder-benefits.html` L348:「権利確定日を過ぎた**翌営業日**が権利落ち日です」→ **誤り**。T+2決済では権利落ち日は権利確定日の**前営業日**（＝権利付最終日の翌営業日）
  - 自サイト2記事で確認: `guide-settlement-cycle.html` L435・`guide-dividend-basics.html` L290
  - 記事内の L383 キャプション「権利付最終日…の翌取引日が権利落ち日」とも矛盾
  - 読者が「権利確定日に買えば権利取得可」と誤解する危険あり
- **推奨修正（1行）**:
  ```
  【誤】権利確定日を過ぎた翌営業日が権利落ち日です。
  【正】権利確定日の前営業日（＝権利付最終日の翌営業日）が権利落ち日です。
  ```
- **Opus 適用済み修正（3件・ドラフトに未反映）**:
  1. figcaption「必ず発生する」→「制度信用取引では上乗せされることもある」（逆日歩は制度信用のみ）
  2. L398「企業も増えています」→「企業もあります」（出典なき趨勢断定の軟化）
  3. L448「制度信用取引」「一般信用取引」初出説明1段落追加
- **次アクション**: ドラフト L348 を修正 → autopublish 再実行（次回自動公開候補になる）
...
- **エスカレ理由**:
  1. **重大誤り①（本文 line 349）**: 「権利確定日を過ぎた翌営業日が権利落ち日」→ **逆**。T+2決済では権利落ち日＝権利付最終日の翌営業日＝権利確定日の1営業日**前**。読者が権利取得タイミングを誤認する実害リスク。なお同記事のSVGとfigcaptionは正しく記述されており、本文だけが図と矛盾している。
  2. **誤り②（figcaption line 445頃）**: 「逆日歩が**必ず**発生する」→ 逆日歩は制度信用のみ（一般信用では発生しない）。SVG本体は正しいがキャプションが断定。
- **対応**: 上記2箇所をローカルで修正→再ゲート（check_guide_draft.py）→ 次回の autopublish サイクルで公開。
- **品質**: ①③⑤=✅、②⚠️（権利付最終日が定義なしで登場）、④⚠️（誤り②に起因するfigcaptionの言い切り）
...
- **基準日**: 2026-08-28（JST）
- **topic**: 「平均でいい」と「平均に勝ちたい」は何が違うのか（インデックス運用 vs アクティブ運用の設計思想）
- **key**: `index-vs-active`
- **シリーズ**: 💰 投資の基礎知識
- **生成ファイル**: `drafts/draft-index-vs-active.html`
- **参照出典**:
  - SPIVA Japan Year-End 2024（S&P Global）: https://www.spglobal.com/spdji/en/spiva/article/spiva-japan-year-end-2024
  - 各種金融機関の信託報酬集計（大和証券・野村AM等の公開資料・複数メディア比較）
  - 複数の投資教育サイトによる費用比較（2024年3月時点）
- **自己コンプラチェック結果**:
  - ✅ 個別銘柄・特定ファンドの売買推奨なし（社名・ファンド名は一切記載していない）
  - ✅ 断定・利益保証なし（「必ず」「絶対」「100%」「保証」等の使用なし）
  - ✅ どちらが優れているという結論を出していない（性質の説明・設計思想に限定）
  - ✅ SPIVA数値は出典明記・「参考情報」として扱い断定していない
  - ✅ kinsho-v1 免責：記事冒頭バナー・本文末・フッターの3箇所に記載
  - ✅ noindex/nofollow メタタグあり（下書き=検索除外）
  - ✅ 信託報酬の数値に取得日（2024年3月時点）と情報源を明記
- **SVG概念図**:
  - ① 設計思想の比較図（インデックス型 vs アクティブ型の特性2列）→ ライト/ダーク実機確認要
  - ② コスト差が長期成長に与える影響（概念図・2本の成長曲線）→ ライト/ダーク実機確認要
  - ③ 生存バイアスの構造図（廃止ファンドがデータから消える仕組み）→ ライト/ダーク実機確認要
- **人間の残作業**:
  1. SVG 3点の実機ライト/ダーク表示確認（色・視認性・モバイル崩れチェック）
  2. タイトル・見出しの微調整（必要に応じて）
  3. 公開は毎朝 08:40 の autodraft-publish が決定論ゲート→Opusコンプラ→品質ルーブリックの順で自動実行
  4. 公開時の案内: `mw publish --file guide-index-vs-active.html --category 投資の基礎知識 --emoji 💰 --card-title "「平均でいい」と「平均に勝ちたい」の違い" --desc "インデックス型とアクティブ型の設計思想・コスト差・生存バイアスを図解。どちらを選ぶかの考え方まで。"`
...
記事の元の数値はすべて verify 緑で誤りではないが、「同じ母集団を20年で測ると有意にマイナス」という
文脈が載っていなかった＝読者が期待値を過大に見積もりうる状態だったため。
...
| 同 2026-01-01〜06-10（ライブ計測の**直前**） | 129 | 27.9% | **−0.339** | [−0.54, −0.14] 🚨有意マイナス |
| 同 2026-06-11〜07-17（ライブ計測期間と重複） | 21 | 52.4% | +0.222 | [−0.39, +0.83] |
| 本文のライブ計測 2026-06-11〜08-25 | 35 | 71.4% | +0.667 | [+0.32, +1.02] |

- 年別: 20年で有意プラスは2017年の1年のみ・有意マイナスは6年
- ホールドアウト(2021-) N=1,238 R−0.021[−0.11,+0.06] は本文のライブ区間と**非重複**
- **リプレイの不具合ではない**: 同一期間で 全件 −0.025 vs −0.033、ロング全体 +0.018 vs +0.016 とほぼ一致。
  食い違いは逆張り買いの筋に限局
- 注記ボックスで「**ライブ計測を始めた日から都合よくプラスに転じている**＝観測窓の選択効果の疑い」と明記し、
  「現時点でエッジがあると読むべきではない」と結論を弱めた

30秒まとめにも一文（新しい%は入れず＝要約検査を汚さない）、§10 まとめにも「長期データの裏づけを持たない」を追加。
guides.html のカードと index の更新履歴も
「〜ただし前向きN=10で事前宣言H2は未達」→「**〜ただし20年リプレイでは期待値マイナス**」に差し替えた。

### ゲート再実行

- `signal_lab_verify.py`（下書き・公開版とも）: **EXIT=0 / 5クレーム緑 / 要約未検証0 / SVG警告0**
- `check_guide_draft.py`: **GREEN**
- `check_site_consistency.py`: エラー0（警告26件は既知）
- ライブ確認: 記事の§7-5見出し・N=4,541・−0.339・30秒まとめの一文／guides カード／index 更新履歴すべて反映
  （index は Pages の CDN キャッシュで数分遅延。リポジトリ側 6febd19 で確認済み）

> ℹ️ 副作用1件: `guides.html` をローカル編集した際に改行コードが CRLF→LF に正規化された（3,340行ぶん約3.3KB減）。
> 記事リンクの欠落が無いことを公開前リモート版との差分で確認済み（差分0）。表示・整合性チェックとも影響なし。

---

## 2026-08-25 | ✅ メール配信に期待値ゲートを追加 [解決済み] | email-positive-ev | technical-alerts

オーナー指示「メールのアラートは実際に検証して、**期待値がプラスになるものだけ**」。実測してから実装した。

### 実測でわかったこと＝通していた方が期待値ゼロだった

メールを出すのは **4H レーンだけ**（1h/1d は `--no-email`＝収集専用）。ところが tracker の昇格判定は
**全時間足をプールした前向き窓**で行うので「プールで昇格＝そのレーンで儲かる」ではない。
4H に切り直して実測（`signal_lab_tracker.stats`＝日付クラスタ補正SE）:

| 仮説 | 4H前向きN | 勝率 | 平均R | RCI | |
|---|---|---|---|---|---|
| `trend=上昇×reversalL`（**唯一メールを通していた**） | 75 | 42.7% | **−0.004** | [−0.232, +0.223] | 🚨 期待値ゼロ |
| `state_maup_long`（**probe欠陥で1件も通っていなかった**） | 176 | 51.1% | **+0.193** | [−0.007, +0.393] | 🟡 点推定+・下限は紙一重 |

さらに**事前登録45仮説すべてを4H足に切り直して判定 → 「期待値プラスが有意」は 0 件**。
（唯一はっきりプラスなのは **1D足 × `trend=上昇×reversalL` の +0.922 RCI[+0.461,+1.382]** だが N=17 で基準未満、
かつ 1D レーンはそもそもメールを出さない。）

### 実装（オーナー選択＝厳格）

`filter_positive_ev(promoted_hyps, timeframe)` を新設。**判定式は自作せず `signal_lab_tracker.stats/judge` をそのまま流用**
＝昇格と同じ基準の単一ソース（`N >= promote_min_n(80)` かつ **平均Rの95%CI下限 > 0**・SEは日付クラスタ補正）。

- 昇格エッジを、**実際にメールを出す時間足の前向き窓**で測り直し、`judge()=="promoted"` のものだけ残す
- `filter` に `tf` を持つ仮説は、その時間足の run 以外では対象外
- **fail-closed**: tracker や signals-log が読めない・計算できないときは **送らない**（厳格の意図に合わせた。
  従来の fail-open〔＝読めなければ全送信〕とは逆向きなので、🚨 を出して理由を必ずログに残す）
- キルスイッチ: `EMAIL_POSITIVE_EV_ONLY=0`
- 却下時は理由（`N<80` か `RCI下限が0以下`）と実測値をログに書く＝なぜ来ないのかが後から追える

### 検証（ライブ・4Hレーン）

[run 32847224073](https://github.com/invest-ai-info/marketwatch-ai/actions/runs/32847224073)（`technical-alerts.yml`＝メールを出す本番レーン）:

```
📐 期待値ゲート: ON — tf=4h の前向き実測で絞り込み（基準は tracker と同じ N>=promote_min_n かつ RCI下限>0）
   🔇 auto_reversal_long-True_trend-上昇: 4h では基準未達（N<80）→ 送信しない — N=75 勝率42.7% 平均R -0.004 ...
   🔇 state_maup_long: 4h では基準未達（RCI下限が0以下）→ 送信しない — N=176 勝率51.1% 平均R +0.193 ...
   （送信対象の昇格エッジ 0 件＝今回 run はメール送信なし・データ収集のみ）
完了 [4H]: 新規シグナル 5 件 / メール 0 通送信 / ログ累計 4728 件
```

単体テストも通過: 4h/1h/1d いずれも0件・fail-closed で `[]` を返す・`EMAIL_POSITIVE_EV_ONLY=0` で素通し2件。

### いまの状態

**アラートメールは送信ゼロ**。ただし直近29日も実績0通なので**実挙動は変わっていない**。変わったのは理由で、
「probe のバグで黙って0通」→「4H足で測ったら基準を満たすものが無いので、理由をログに書いた上で0通」。
基準を満たす仮説が現れれば**コード変更なしで自動的に再開**する（毎run 測り直すため）。

📌 次に効きそうな一手（未実施・要判断）:
1. `state_maup_long` は 4H で N=176・平均R +0.193 と点推定は明確にプラス。**RCI下限 −0.007 は紙一重**なので、
   N がもう少し積めば基準を満たす可能性が高い＝そのまま蓄積を待つ
2. 1D足 × `trend=上昇×reversalL` が +0.922 RCI[+0.461,+1.382] と唯一はっきりプラス。**N=17 で基準未満**だが、
   ここが N≥80 に達したら `technical-alerts-1d.yml` の `--no-email` を外す価値がある
...
オーナー指示によりメインClaudeが台帳 `signal-lab-ledger.md` と `signals-log.json` から再計算して修正した。

| 指摘 | 誤 | 正 | 根拠 |
|---|---|---|---|
| [A] 図1の1H足ラベル | 1H足 54.7%（#069・N=179） | 1H足 47.0%（#069・FWD×1H N=115・E(R)=+0.095） | 54.7%/179 は #069 の **FWD全体**（1H+4H+その他の混在）。台帳 #069 行に「FWD×1H 54/115=47.0% CI[38.1%,56.0%] E(R)=+0.095」と明記 |
| [B] §4 FWDグループ内訳 | index3+jpy_fx5+other_fx1+metal2+^SOX1=12 ≠ FWD総数10 | metal を削除し 3+5+1+1=10（k=2+4+1+0=7） | signals-log.json の FWD 10件を明細照合。金属(GC=F/SI=F)は FWD 期の発火 **0件**＝7件すべて IS 期 |
| [C] §7-1 の N=312 | #069: N=312 / #074: N=312 | #069: 全期間 N=312（IS133+FWD179）／#074: 全期間 N=353（IS133+FWD220） | 台帳 #069「全期間 150/312」・#074「IS:133 / FWD全体:220」 |

**派生修正（[B] の誤読防止）**: 「金属がアウトライヤー」は **IS 側だけの話**。FWD 70.0% に金属は寄与していない旨を §7-2 に追記。
§5 の「金属と拡張^SOX（各1件loss）」も件数誤記だったため「金属(N=7)と拡張^SOX(N=1)を除いた5グループ 22/27＝81.5%」に訂正。

**Opus 軽微グレー5件も併せて対応**: ①30秒まとめ「確認できた」→「95%CI下限ベースでは上回る結果となった」 ②図2キャプションに RSI の95%CI下限41.7%<43% を補足 ③§5「運用を検討する際の参考」→「今後の検証で優先的に確認すべき論点」 ④§7-2「他グループは66%以上」→「^SOX（N=1）を除く他グループ」 ⑤footer の免責 `<p>` に `data-disclaimer="kinsho-v1"` 付与（3箇所に）。

**再ゲート結果**: `python signal_lab_verify.py draft-signal-lab-079.html lab-079-claims.json` → **EXIT=0 / 5クレーム全緑 / 要約未検証0件 / SVG警告0件**（claims.json は変更なし＝数値の実体は元から正しく、誤っていたのは本文・図・参照だけだった）。

**残作業（人間）**: Opusコンプラ再実行 → 独立Opus白確認 → `finalize_signal_lab.py 079` → `publish_article.py` → sync → ライブ確認。**公開はオーナー承認待ち**。

> 🔎 **番人の死角を1件発見（別件・未修正）**: `check_automation_health.py` ⑧ の見出し正規表現 `^##\s*(\d{4}-\d{2}-\d{2})\s*\|` は
> 日付の直後に `|` が来る形しか拾わない。8/25 の #079 エスカレ見出しは `## 2026-08-25 signal-lab: 🚩 要人間レビュー | ...` と
> 日付の後に文字列が入る形だったため **⑧ の集計に入っていなかった**（8/25 のレポートは「未対応の 🚩 は 0 件」と報告）。
> ＝エスカレ滞留の番人が signal-lab レーンのエスカレを見落とす構造。本エントリは正規表現に合う形で書いている。
...
## 2026-08-24 ✅公開 | signal-lab-daily #078 RSI売られすぎ逆張り買い FWD N=232 WCI下限46.2%突破

- **記事番号**: #078
- **テーマ**: RSI売られすぎ逆張り買い FWD N=232——WCI下限46.2%で損益分岐突破・4H足66.7%エッジ確立・1H足44.6%は無エッジ
- **公開ファイル**: `guide-signal-lab-078.html`
- **claims**: `drafts/labnotes/lab-078-claims.json`（10件）
- **公開**: 2026-08-24（ゲート全通過: verify EXIT=0 / Opus🟡軽微修正→白 / 独立Opus INDEPENDENT_OPUS_OK）

---

## 2026-08-23 🚩要人間レビュー（コンプラ🔴：事実誤認＋kinsho-v1不足） | signal-lab-daily #077　[解決済み]（2026-09-24 オーナー判断で公開見送り＝コンプラ承認ではない）
...
- **公開**: なし（🔴のため）
- **決着（2026-09-24・オーナー判断）**: 🗂 公開見送り。同じ仮説（state_maup_long）の後日版 #084（2026-08-30 公開）が「合格取り消し」まで扱っており、8/23 時点の「1回目の合格」は既に覆っている。さらに期間の区切りが登録日当日（7/20）で、SOP どおり翌日（7/21）にすると数字が全部変わる。下書きと labnotes は保持・C1/C2 は未修正。🔖 見出しの `[解決済み]` は番人（automation-health §⑧）を黙らせるための印であって、コンプラ審査を通ったという意味ではない。公開する場合は期間の区切りを 7/21 に直して数字を取り直し、正規手順を通して印ごと書き直すこと。
...
- **優先度**: ①（tracker[v] ⛔反証変化）
- **IS/FWD**: IS 135/377=35.8% E(R)=-0.284 → FWD 273/571=47.8%; cluster補正 E(R)=+0.123 RCI[+0.01,+0.23]
- **claims.json**: 16件（全IS/FWD分離済み・fired_before/fired_from境界=2026-06-25）
- **下書き**: `drafts/draft-signal-lab-072.html`
- **verify.py**: 🟢EXIT=0（16/16クレーム一致・30秒でわかるOK・SVG OK）
- **Opus コンプラ**: 🔴黒——期間ラベル誤記（IS列に全期間値を使用）
- **エスカレ理由（詳細）**:
  - **黒①** テーブル 4-1「IS 勝率」列が実際には全期間値（fired_before フィルタ不使用の k/n）を表示している
    - 金属: 表示 36.0%（全期間 86/239）→ 真のIS 22.8%（31/136, fired_before 2026-06-25）
    - 指数: 表示 〜44%（出典不明）→ 真のIS 56.0%（14/25）; FWD=51.0% ∴ **改善ではなく低下（-5.0pp）**
    - 記事の主張「指数グループは全期間を通じて安定して改善」が正反対になる
  - **黒②** SVG figure 2「金属IS」バーが 36.0%/N=239（全期間値）を描画 → 真値 22.8%/N=136 が必要
  - verify.pyはSVG overflow検査のみ（数値の正誤は検査対象外）のためEXIT=0でも上記誤りは通過する
- **必要な人間修正（再公開前に全て必須）**:
  1. `drafts/labnotes/lab-072-claims.json` に各グループの真のIS期間クレームを追加（fired_before フィルタ付き）
     - 金属IS: k=31, n=136（要再計算確認）
     - 指数IS: k=14, n=25（要再計算確認）
  2. `drafts/draft-signal-lab-072.html` テーブル 4-1 の「IS 勝率」列を真のIS値に修正
  3. SVG figure 2「金属IS」バーの 36.0%→22.8%、N=239→N=136 を修正
  4. L462 付近「指数グループは...安定して改善」→ 実際はFWD低下（-5.0pp）の記述に修正
  5. kinsho-v1 免責が3箇所（バナー・本文・フッター）に揃っているか確認
  6. 修正後: `signal_lab_verify.py` → Opus コンプラ → 独立Opus → `publish_article.py` を再実行
- **参照**: #067 同種エスカレ（IS/FWD列混同）の再発——ゲート強化要検討
- **次回routine**: 人間修正後の再ゲートはルーティンを使わず手動実行を推奨
...
## 2026-08-16 | ✅ 公開完了（エスカレ解消） | signal-lab-daily #071

- **対象ファイル**: `guide-signal-lab-071.html`（公開済み）
- **テーマ**: S/R壁あり×ショート 前向きN=142——IS 58.5%→FWD 33.8%、後半22.0%と⛔反証接近
- **解決**: Q1/Q2/Q3クレーム追加(19/19 GREEN)・数値修正・Opus12件グレー修正・独立Opus🟢・公開完了
...
- **claims**: 17件（`drafts/labnotes/lab-071-claims.json`）
- **signal_lab_verify**: ✅ GREEN 17/17
- **Opus コンプラ判定**: 🔴黒（法的問題なし、但し数値整合不備あり）

### 🔴 黒①: H1・title・meta に「FWD 18.8%」（未verify・独立再計算と不一致）
- H1/title/meta に「IS 58.5%→FWD 18.8%」と記載あり
- この 18.8% は claims.json に存在しない（verify スコープ外の Q3 三等分値）
- Opus 独立再計算では Q3(7/31〜8/14)=10/48=**20.8%**、中盤(7/14〜7/28)=20/47=**42.6%**（記事は 9/48=18.8%, 21/47=44.7%）
- 影響箇所: title(10行)、meta description(11行)、H1(180行)、figcaption(585行)、641行、670行
- **SVG にも 44.7%が残存**（444, 452, 454行）

### 🟡 グレー①: 「ほぼ確実な負け越しグループ」（411行）
- oil N=9 に対し断定度が過剰（小サンプル警告を追加すべき）

### 🟡 グレー②: 「避けるべき」（676行）
- 「〜べき」は取引行動指示語。→「統計的に劣位」等に軟化

### 🟡 グレー③: 「〜する方が合理的」（532行）
- 比較優位示唆。→「相対的に良好な数値を示している」等に軟化

### 人間への対応依頼
1. **三等分区間の数値を確定**: `drafts/labnotes/lab-071-analysis.md` の Python スクリプトを `fired_at` ベースの日付フィルタ（`fired_from`/`fired_before`）で再実行し、正確な Q1/Q2/Q3 の k/n を確定
2. **claims.json に三等分クレームを追加**: 確定した k/n で `fired_from`+`fired_before` フィルタのクレームを追加（または三等分表示を削除）
3. **H1・title・meta・figcaption・SVG を確定値で更新**（FWD全体=33.8% を主数値とし、部分区間は日付明示）
4. **グレー 3 件を表現軟化**（411/676/532行）
5. `python signal_lab_verify.py drafts/labnotes/lab-071-claims.json` で再検証
6. コンプラ Opus 再審査
7. 独立 Opus 白確認 → 公開

> **Opus コンプラコメント**（抜粋）: 金商法・景表法・免責・AdSense の観点だけであれば、修正 2〜3 箇所で 🟢白に到達できる防御の厚い良質な研究記録です。しかし H1・title・meta の「FWD 18.8%」と本文の「中盤 44.7%」が claims.json に存在せず独立再計算とも一致しないため、数値変更を伴わない 🟡自己修正レーンでは処理できません。
...
## 2026-09-12 BOOKWATCH 掲載見送り メモ

**見送り候補**: 『AIに9割まかせる ほぼほったらかし株式投資 株価が3.53倍、利益253万円に爆増！』
- **著者**: 投資家Go
- **出版社**: KADOKAWA
- **発売日**: 2026年9月11日
...
**決定論ゲート（signal_lab_verify.py）**: 🟢 GREEN（6/6 緑・SVG警告0）
→ ⚠️ **ただし緑は「見落とし」**。`lab-106-claims.json` が期間キー（`fired_before`/`fired_from`）を1つも使っていないため、`period_label_check` が SOP の限定①により**スキップ**された。§5 の IS/FWD 6数値はオラクルの検証範囲外。

**コンプラ判定**: 🔴 **黒**（SOP 2026-08-12 の #067 と同型＝景表法上の優良誤認）

**黒の理由（記事内で「IS」の定義が2つあり矛盾）**:
1. `lab-106-claims.json` の全6クレームが**全期間の集計なのに「（IS）」とラベル**されている（filter に期間キーなし）。SOP 明記＝「全期間の数字に『（IS）』とラベルするのは景表法上の優良誤認＝黒」。
2. 本文 §1（L286）「IS（in-sample）は**全期間の集計**、FWD は 2026-08-11 以降」＝ IS と FWD が重複する定義。
3. 一方 §5（L467）は「IS（〜2026-08-10）N=65」＝ 登録日で分離した定義。→ 同一記事内で IS＝492/102件系 と IS＝65件 が併存。
4. §3 末尾（L405）「4H足×上昇中の67.4%は…**IS段階の値**」＝ 67.4%(N=43) は IS(N=25)+FWD(N=18) の**全期間値**。IS と名乗るのは誤り。読者は「前向き検証前の値」と誤認する。

**FWD 境界の1日ズレ（別件・要判断）**:
- トラッカー `rsi=os×trend=上昇` の `registered_at` = **2026-08-11**。SOP は FWD＝`fired_from <登録日翌日>`＝**2026-08-12**。
- 記事は FWD＝2026-08-11〜（登録日当日を含む）。独立再計算：
  - 記事の境界(08-11)：IS 43/65=66.2% ／ FWD 18/37=48.6% ← **記事の6数値はこの境界で完全一致（数値自体は捏造なし）**
  - SOP の境界(08-12)：IS 44/66=66.7% ／ FWD 17/36=47.2%
- 登録日当日の1件（勝ち）が FWD に入っており、**FWD がわずかに有利側**に出ている。

**人間への依頼**:
1. claims.json の「（IS）」ラベルを実態に合わせる（全期間なら「全期間」、IS を主張するなら `fired_before` 付きクレームを追加）
2. 本文 §1 の IS 定義文と §3 末尾「IS段階の値」を、§5 の定義に統一する
3. FWD 起点を 2026-08-11 と 2026-08-12 のどちらで確定するか決める（SOP 準拠なら 08-12・数値差し替えが必要）
4. `signal_lab_verify.py` の限定①（期間キー未使用なら period_label_check スキップ）を塞ぐか要検討＝**本文が IS/FWD を語るのに claims が期間キーを持たない記事**を RED にする案

**適用済みの🟡グレー修正（表現のみ・数値/SVG/表/免責は不変）**: 3件（下記セッション報告参照）

---
## BOOKWATCH 2026-09-26 除外メモ

- 『AIに9割まかせる ほぼほったらかし株式投資 株価が3.53倍、利益253万円に爆増!』（KADOKAWA・著者：投資家Go・2026-09-11発売）
  - 除外理由：副題「株価が3.53倍、利益253万円に爆増!」が投資成果の訴求表現（煽り本基準）に該当。著者が投資スクール運営者（教材商法との境界）。内容自体はAIと株投資の入門解説と見られるが、BOOKWATCH掲載基準の「煽り本除外」を適用。
