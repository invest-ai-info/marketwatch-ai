...
- 🔒 **🔴黒のときの追加処理（2026-07-26）**：このリポジトリは **public** で `raw.githubusercontent.com` から
  **誰でも下書き本文を読める**（サイト側の `noindex`/robots.txt は GitHub には効かない＝HTTP 200 で実測）。
  黒と判定したら **`drafts/draft-<key>.html` を削除コミットし**、`REVIEW.md` には**理由の要約だけ**を残す（本文・引用を書かない）。
  ⚠️ **このレーンの下書きは前日 05:30 の autodraft-article が生成した時点で既に public** なので、削除しても
  **git履歴には残る＝完全な取り消しにはならない**。恒久対策（下書き自体を公開リポジトリに置かない設計変更）は別途検討中。
  ※🟡グレー/要協議/ゲート赤/重複 など**法務リスクでないエスカレは従来どおり**下書きを残してよい。
