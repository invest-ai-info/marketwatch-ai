...
## 2026-08-25 追記2 — §8 をトラッカー実測値に合わせた

`signal_lab_tracker.py update --date 2026-08-25` 実行後の `signal-lab-tracker.json` 実測値:

```
tf=1d×reversalL (id=auto_reversal_long-True_tf-1d, registered_at=2026-07-15)
  forward: k=7 n=9  77.8% CI[45.3,93.7] avgR=+0.815 RCI[+0.037,+1.592]
  alltime: k=25 n=34 73.5% CI[56.9,85.4] avgR=+0.716 RCI[+0.371,+1.060]
  status: tracking
```

記事 §8 は当初これを **verify.py の値（7/10・70.0%・RCI[-0.065,+1.332]）** で書いていたが、
見出しが「トラッカー現況」なので**台帳の実測値に差し替えた**。

差の正体は**ユニバースの扱い**（境界日の扱いではない）:
- verify.py: 拡張ユニバースを除外しない → FWD N=10 / 全期間 N=35
- signal_lab_tracker.py: 標準ユニバースのみ → FWD N=9 / 全期間 N=34
- 差分は ^SOX 1件（2026-07-28・bb_lower_touch・**sl**）。loss なので k は 7 / 25 のまま不変。

⚠️ この1件で **平均R の RCI 下限が −0.065 ↔ +0.037 と符号をまたぐ**。
昇格判定の核心が母集団定義1件で反転する規模＝「RCI下限がプラス」は結論として扱えない。
§8 に注記として明記した。
