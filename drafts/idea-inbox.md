...
## 2026-08-30（JST）
### slug: tsmom-cross-asset
- 名前: 時系列モメンタム（クロスアセット・自己継続型）
- 主張: 各資産の過去12ヶ月リターンの符号（正/負）が翌月リターンを予測する傾向がある【出典の主張・未検証】。Moskowitz, Ooi, Pedersen（2012, JFE）が株式・債券・通貨・コモディティ先物58銘柄で年率シャープレシオ約1.0を報告。銘柄間の相対比較（クロスセクション・モメンタム）とは異なり、各資産が「自分自身の過去」に対して継続するかどうかを問う別仮説。市場クラッシュ局面での保険効果も確認されている。
- 出典: https://www.sciencedirect.com/science/article/pii/S0304405X11002613 / https://quantpedia.com/strategies/time-series-momentum-effect
- 検証案: signals-logの18銘柄（GC=F/CL=F/ES=F/USDJPY等）で過去252日間（約12ヶ月）リターンの符号別に翌月勝率・期待値を算出。正→ロング、負→ショートの仮想PnLを集計。価格データのみで完結。momentum-12-1（クロスセクション・tested）との比較も行う。
- タグ: ◎

### slug: fx-carry-uirp
- 名前: FXキャリートレード（金利差アノマリー・UIRP不成立）
- 主張: 高金利通貨を買い・低金利通貨（円）を売る戦略が統計的に正のリターンを生む傾向がある【出典の主張・未検証】。カバーなし金利平価（UIRP）が成立すれば利回り差は為替変動で消えるはずだが実証的に不成立（フォワードプレミアム・パズル）。MDPI Risks 2025掲載論文が円キャリーとリスク指標の関係を定量化。2024年8月の円キャリー巻き戻しは規模を示す実例。通常局面の累積リターンは正だが損失は急速に発生するテールリスクあり。
- 出典: https://www.mdpi.com/2227-9091/14/3/46 / https://www.amro-asia.org/wp-content/uploads/2024/12/20241219-Analytical_Note_Carry_Trade.pdf
- 検証案: USDJPY/EURJPY/GBPJPY/AUDJPYについて米・欧・英・豪の政策金利と日銀政策金利の差分を月次で計算し、金利差拡大局面（キャリー積み増し）vs 縮小局面（BOJ利上げ）の翌月リターン平均・勝率を比較。
- タグ: ○

### slug: qmj-jp
- 名前: クオリティ・マイナス・ジャンク（QMJ・日本株版）
- 主張: 利益性・成長性・安全性・配当性向の4次元z-scoreを合算した「クオリティスコア」が高い株は低い株より高いリターンを示す傾向がある【出典の主張・未検証】。Asness, Frazzini, Pedersen（AQR 2014・Review of Accounting Studies 2019）が24カ国中23カ国で有意な正リターンを報告（日本含む）。piotroski-fscore（財務健全性9指標・queued）とは軸が異なり、buffett-quality（5基準スクリーン型・tested）とも指標定義・重み付けが異なる連続スコア型の別仮説。
