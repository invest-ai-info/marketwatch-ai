...
WORKFLOW_CHECKS = [
    ...
    ("EDINET有報（話題の企業）", "edinet-yuho.yml",        80, "warn"),
    ("経済カレンダー検証",       "verify-calendar.yml",   200, "warn"),   # 月曜07:10＋毎月25日
    ("EDINET大量保有",          "edinet-holdings.yml",     80, "warn"),   # 同じ平日1回・同日に登録（監視漏れだった）
    ...
    # 🆕 2026-09-17: 朝の指標ダイジェスト。平日朝だけなので週末をまたぐ 80h で見る。
    #    🚨 これが黙って止まると「指標を知らせる仕組みがある」という前提だけが残って一番危ない。
    # 🆕 2026-09-17: 指標メール2本。黙って止まると「知らせる仕組みがある」前提だけが残って危ない。
    #    ⚠️ digest は routine の push に相乗りする設計なので、**routine が止まると道連れ**。
    #       cron の保険もあるので 80h（週末をまたぐ平日朝の間隔）で見る。
...
# ⑭ ワークフロー連鎖（workflow_run）が実際に効いているか。
#
# 🚨 2026-09-17 の発見: update-market-news は `paths: jp-rankings.json` で
#    「夕方更新を遅延ゼロで反映」と書かれていたが、**一度も発火していなかった**。
#    GitHub は `GITHUB_TOKEN` で押されたコミットからワークフローを起動しない仕様で、
#    その2ファイルを押しているのは Actions（jp-rankings.yml）だったため。
...
def main():
    ...
    for label, wf, max_days, since in CHAIN_WATCH:
        try:
            ...
            continue
        if st == "ok":
            body.append(f"- ✅ 🟢 {label}: {msg}")
        elif st == "waiting":
            body.append(f"- ⚪ {label}: {msg}")
        else:
            body.append(f"- 🚨 🟡 {label}: {msg}")
            bad.append((f"{label} の連鎖", "warn"))
    body.append("")
