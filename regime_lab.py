...
🔁 2026-09-26 オーナー決定: 幅の出し方を保守版（exit_rule_backtest._mean_se＝_mean_se_safe）に切り替えた。上の較正（p値の裾が
           名目の約2.3倍）はこの切り替えの前のもの。判定の基準（ボンフェローニ・0.10R・前後半）は変えない。切り替え前の結果でも
           主の比較で「差がある」は0だった＝幅が広がっても結論は変わらない（p値は大きくなる方向）。
