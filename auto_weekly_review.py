...
- 「先週データからの気づき」を箇条書き 3 項目: **先週の数値そのものの解釈に限定**し、将来の行動を示唆しない。
  例「・反転検知ありシグナルは 5 件すべて SL となり、当該条件下の勝率は 0% だった」
- 「集計上の留意点」を箇条書き 2 項目: サンプル数の少なさ・期間バイアスなど、**この数値をどこまで信頼できるかの限界**
        ...
        try:
            ...
            if text:
                ...
                if bad:
                    ...
                    # 全滅したら決定論テンプレ（_fallback_lessons）へ落とす＝**黒は公開しない**。
...
#    実際に「EURUSD=X …同銘柄での取引を見送ることを強く推奨します」「SI=F …ロットを減らして」
...
<body>
<header><div class="header-inner"><div class="header-title"><svg viewBox="0 0 96 96" style="width:27px;height:27px;vertical-align:-4px;margin-right:2px" aria-hidden="true"><rect x="2" y="2" width="92" height="92" rx="21" fill="#0969da"/><polyline points="16,72 34,50 50,58 70,32" fill="none" stroke="#ffffff" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"/><circle cx="74" cy="27" r="10.5" fill="#d92b2b" stroke="#ffffff" stroke-width="4"/></svg> MarketWatch AI</div><div class="header-meta">日本人投資家のためのマーケット情報サイト</div><div style="margin-top:11px;padding-top:11px;border-top:1px solid rgba(128,128,128,.22)"><div style="font-size:1.3rem;font-weight:700;color:#1E3A6E">📚 解説記事</div></div></div></header>
<main>
<nav class="nav-bar">
