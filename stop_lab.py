...
前向きの登録（🆕 2026-09-26・**本物のデータの結果を見た後**・オーナー指示「4つとも登録して前向きに検証して」）:
         探索では「差がある」が0だったため、決まりどおりなら前向きの判定対象は無い。オーナーの指示に従い、最も有力な候補を
         1つだけ結果を見た後に登録する＝「日足の逆張り（押し目買い）は、損切りを ATR の3倍にすると 1.5倍より良い」
         （A30−A15 +0.046R p=0.0010・前半 +0.050／後半 +0.042・4時間足 +0.051［+0.01,+0.09］・偽薬 −0.005＝
         ランダムな入口では出ない。ただし差 0.10R 未満で基準外）。lab_forward の決まり（件数100ごと・95%幅が0をまたがず
         同じ向き・差0.10R以上で合格・合格2回連続で確定）で判定する。⚠️ 差が0.10R 未満なら前向きでも「合格」にはならない
         （決まりは緩めない）＝「同じ向きだが小さい」は「まだ判断できない」と出る。日足の逆張りは18銘柄で年に約250件。
         順張りは「損切りの幅でほとんど変わらない（±0.015R 以内）」＝差が無いことは、この仕組みでは確定できないので参考表示のみ。
...
def evaluate(ts1d, ts4h, pl1d, pl4h, fwd1d=None, prev_forward=None, today=None, fwd4h=None):
    ...
    if fwd1d is not None:
        for r in primary:
            ...
            if r["flag"] or k in FWD_REGISTERED:
                forward[k] = LF.step((prev_forward or {}).get(k), d, RL._sign(r["diff"]) or 1, today)
