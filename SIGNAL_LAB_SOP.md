...
- **頻度**＝**週1回まで**（`exit-lab.json` がその週に更新されていて、①昇格/反証・読者の質問が無い日）。1回に**1つの入口×1つの足**だけ扱う。順番＝日足の8入口 → 4時間足の8入口 → 以後は「目立つ組の前向き」の定点観測。どれを扱ったかは台帳の行に `出口-<足>-<entry>-<side>` と書く（同じ組を2回続けない）。
- **書き方**:
