...
<!-- SESSION_HANDOFF スリム化（2026-08-06）で退避 -->

## 📦 2026-08-08 退避（ブランド色 Phase 1 の落とし穴・詳細）

> **「名指しの形が実体と違う」3種の全文**（記事は塗れているのに一部だけ旧色、という気づきにくい壊れ方）:
> ①生成スクリプトの CSS は f-string ＝**波括弧が二重** ②色は CSSブロックでなく**要素の `style` 属性**にも書かれている
> ③クラウドの新テンプレは **CSS変数 `var(--accent)` 経由**。いずれも `fstring_form()`／インライン見出しルール／
> 変数ルールで**コード側に解決済み**。疑うときは「どこに書かれた色か」を先に数える
> （`<style>`内 / f-string内 / style属性内 / CSS変数）。
> h1 の font-size 実測内訳＝1.95rem 180本／1.85rem 51本／1.8rem 47本。

## 📦 2026-08-06 退避（完了節の圧縮残骸・旧入口ナラティブ）

> ### 📚 7/31 夜の3件の要約（全文は【2026-08-01 退避】節）
> - **(d)** DOCTRINE 予算を 30/36KB へ改定（オーナー承認・根拠は `_doctrine_check.py` の `DOCTRINE_WARN_KB` 直上コメント）
> - **(e)** 仮説3本（Q10/Q24/Q27）は走らせる前に設計が成立しないと判明して全て未実行。恒久対策＝`_precheck_feasibility.py`。
>   Q24 の前向きtracker登録だけがオーナー判断待ち（変種9本 or 主張1本／チェックポイントN）→ **宿題表 #10 へ昇格**
> - **(f)** 不要物の棚卸し＝ほぼ何も無かった（11件21KB削除）。価格キャッシュは消さない（Yahoo遡及上限超は再取得不能）→ **絶対遵守へ1行移設**
>
> ### ✅ (a) 為替介入＝記事公開で決着（8/3・ライブ確認済み・全文は【2026-08-05 退避】節）
> 持ち越す教訓1つ＝**外挿の適用限界を🔴で自己申告する型**（独立Opus監査3巡で私の誤り4件を捕捉。うち最重要は
> **n=7の標本に協調介入が1件も無いのに外挿**した件）は速報記事へ横展開する価値あり。
> **(b) Search Console の404レポート**＝優先度低（7/31 に全288ページ再監査済みでサイト内404はゼロ）。
> 残るのは再クロール待ち/外部被リンク/削除済みのいずれか＝別のサイト内URLが載っていたらそれだけが未知の穴。
>
> ### (c) 日本株の蓄積（圧縮前の全文・handoff には1行要約を残置）
> 23営業日/1,053行・カバー率95.8%。日数ゲート(≥20)は達成だが各群N≥100は H2/H5 の2つだけ＝本検定は事前登録どおり自動見送り中。
...
>
> ### ⚠️ 夜の回線輻輳は「日による」
> `api.github.com` は IPv4 のみ。決めつけず `Test-NetConnection api.github.com -Port 443` で実測してから判断
> （詳細=auto-memory `reference_github_api_ipv4_congestion`。押し込みは `sync_to_github.py --batch`＝ルール表参照）。
>
> ### 🔜 旧「次セッションの入口」ナラティブ（2026-07-27 22:20 時点）
> 在flight（未完了で手が止まっているもの）はゼロ。7/27 は Q33〜Q43 の11本＋夜に Q44登録・トラッカー実バグ修正・
> 日本株4本を着地させた。明朝いちばんに見るのは1つだけ＝「Issue が来ていないか」（`check_automation_health.py` §⑥ が
> 09:30 に自動判定）。→ **その後 Issue 無し＝Q35の3件＋Q44 のトラッカー登録は成功で決着済み。**
>
> ### （記録）CLAUDE.md スリム化（2026-08-06・予算32KB内へ）で削った逸話
> - robots.txt 手動編集の消滅実例（原文）:「実例＝2026-06-05 23:48 手動追加→7分後の Actions で消滅、以後~2か月不在なのに CLAUDE.md は「Disallow 済」と書いていた（文書だけ直しても意味がない典型）」
> - robots 実測記録: 遮断3対象（/drafts/ 78本・/memory/・直下.md 21本）は 2026-08-01 実測で live HTTP 200／sitemap 235 URL に .md 混入0
> - ほかは重複の解消のみ＝jp-rankings / news-ticker の表セル詳細（SYNC禁忌節と同文）・更新履歴の旧 push-out 手順（2026-07-28 に誤りと訂正済みの残骸）・ナビ10ボタン化の経緯（2026-06-15）
>
> ### （記録）handoff から除去した旧 declutter コメント3本
> - 2026-07-27 22:20: 7/27の完了節6本（⓪-💰Q42/Q43コスト構造・⓪-🧱建玉/出口/損切り4本・⓪-🚪Q36出口・⓪-⚪B2信頼度・⓪-🎯Q34レジーム・⓪-📐Q33週足ピボット）を SESSION_ARCHIVE.md へ退避
> - 2026-07-28: 7/26完了節3本（⓪-🔒コンプラ／⓪-⏱デイトレ準備／⓪-⏭自社株買い見送り）を【2026-07-28 退避】へ移動。退避前に本文へ埋もれていた未解決2件を宿題 #7・#8 へ昇格済み
> - 2026-07-27: 7/26完了9節（FXism数式確定/保留29件棚卸し/declutter+キュー予算/Q31/Q30/Q29/watch10件/autopublishキュー/完了4件）を【2026-07-27 退避】へ移動
...
## 【2026-07-31 退避】SESSION_HANDOFF「次セッションの入口」の完了行 #0 / #0b / #0c / #0c2（2026-07-27 分）

いずれも 7/27 に着地済み。要点は auto-memory（project_signal_edge_research / project_masters_queue）と
`research/hypothesis_queue.md` の Q44 ブロックに反映済み。退避理由＝SESSION_HANDOFF が予算30KBに到達したため。

| # | 内容 | 状態 |
|---|---|---|
| 0 | ~~Q44＝「守り6ゲート×1h」を事前登録~~ → **7/27夜に着手前チェックで却下（537営業日必要）＝バックログへ。Q番号は発行していない**。残った実務＝**業者の実スプレッドを取得して `SPREAD_PCT` を実測に置換** | ✅判断済み |
| 0b | ~~BTC×1h にQ番号を発行するか~~ → **オーナー判断済み＝Q44として事前登録（SHA256凍結）＋トラッカー `btc_1h_gate` 登録＋`btc_all_1d` を tf=1d へスコープ補正**。判定は**前向きN≥91到達時に1回だけ**（≒2026-10上旬）。遡及 N=91 勝率31.9% グロス −0.2564 CI[−0.452,−0.060]／他17銘柄との差 −0.3477 CI[−0.584,−0.111]／総コストの24.3%を単独消費。週末アーティファクトではない（平日のみで −0.3575 と悪化） | ✅登録済み |
| 0c | ~~`signal_lab_tracker.py` を push~~ → **2026-07-27 19時 sync 完了（242/242成功）・リモートに `REGISTER_2026_07_27`／`TF_SCOPE_FIX_2026_07_27`／`btc_1h_gate`／`existing_ids` の反映を実測確認**。**明朝の確認は人がやらない＝番人にした**（7/27 19:47・`check_automation_health.py` §⑥）。実装直後にライブ状態で試験し、宣言32件中**まさにこの4件だけ**を欠落として検出・正常スキップ4件は誤検知しないことを確認済み | ✅sync済＋番人化済み |
...
## 【2026-08-01 退避】7/31 夜の3節（(d) DOCTRINE予算改定 / (e) 仮説3本の着手前チェック / (f) 不要物の棚卸し）

要点は SESSION_HANDOFF に1行ずつ残し、全文はここへ。恒久記録は
`_doctrine_check.py` の `DOCTRINE_WARN_KB` 直上コメント（予算根拠）と
`research/_precheck_2026-07-31.md`（Q10/Q24/Q27 の全証拠）にもある。

> ## 🆕 (e) 仮説3本（Q10→Q24→Q27）を順に着手前チェック＝**3本とも実行せず**（7/31 22:00）
> **いずれも「やってみて駄目」ではなく「走らせる前に設計が成立しないと分かった」**。却下の型が3本とも違う。
> 全証拠＝`research/_precheck_2026-07-31.md`（10.9KB）。**結果=R/勝率/pは一切見ていない**。
> - **Q10＝対照が作れない**（同日クロスセクション `min_ctl=30` で使える日数 **0/661**＝19銘柄で理論上限18）⇒ **⏳→💬**
> - **Q24＝入力が無い**（BTログ2本とも `news_count` 保有 **0.0%**）。残る一手は前向きtracker登録のみ＝**オーナー判断待ち**（変種9本 or 主張1本／チェックポイントN）⇒ **⏳→💬**
> - **Q27＝着手条件が未達**（`_regime_targets.py` の抽出は 7/25 と同じ3件のまま）＋Q34❌が §0-11 で期待値低下を明記 ⇒ 見送り継続（⏳のまま）
> ⚠️ **副産物＝第4類型の実例**: ローカル `signals-log.json` **1,028件(6/18止まり)** vs リモート **3,079件(7/31)**。
> 逆に `signals-log-backtest*.json` は**リモート404＝ローカルが真実**。**入力ごとに真実の所在が逆転する**ので毎回突合する。
> ✅ **恒久対策は実装済み**＝`_precheck_feasibility.py`（下表に1行）。
>
> ## 🧹 (f) 不要物の棚卸し＝**ほぼ何も無かった**（7/31 22:30）
> ジョブ系は**休眠ゼロ**（Actions 19本・Windowsタスク9本とも稼働）。孤立ファイルは12件・34KBのみで残り76MBは全て参照あり。
> 実施＝**11件21KB削除**（再生成ログ3件＋6月の使い捨て8件。`_q30_test.py` は Q30棄却の再現手段なので保持）／
> **バックログの死に4行を退避**＝EAP・COT・PEAD の**3行が「着手可」と書いたまま実行・棄却済み＝在庫の嘘**だった。research/ 124→114。
> 🚨 **やらなかった＝価格キャッシュ50MBの削除。「再取得可能」は私の誤り**＝Yahooの遡及上限は **1h=730日/15m=60日**で、
> `_all1h_prices.json` は **329日ぶん**・`_fx1h` は296日ぶんが二度と取れない（`_all15m` は取得スクリプトすら無い）。
> **判定はスクリプトの有無でなく API の遡及上限で行う。**
> ⚠️ `MarketWatch_JP_OpenLog` が 7/31 12:01 に **267014（強制終了）**＝7/23 の永久欠測と同型。**8/3 の次回実行で埋まったか確認**。

> ## 🆕 (d) DOCTRINE 予算を 24/28KB → **30/36KB** に改定（7/31 21:30・**オーナー承認済み**）
> 固定オラクルの緩和なので理由を残す。**減量では解決しないことを実測してから**の判断＝
> ①§3 は既に4行1.63KB＝**退避先として枯渇**（警告文が名指ししていた先が空）／②12行を統合しても
> **0.01KB** しか減らない／③アンカー prose 44件中 **36件が単一出現**＝本文の数値はほぼ全て load-bearing／
> ④アンカーに触れず退避できる最大量は §4+§3 の約2.4KB＝**知見18行＝1作業サイクルで元に戻る**。
> ⇒ 旧予算下ではこれは「行動可能な黄信号」でなく**行動不能な警告**＝`QUEUE_WARN_KB` 14→36（7/26）と同じ構図。
> 加えて **research/ は git 管理外**（同期はAPI経由の `sync_to_github.py`・DOCTRINE は SYNC外）＝
> **削除は復元不可**。数値を満たすために版管理の無い知見を消すのが最も危険な選択肢だった。
> 退避先＝新設 `research/DOCTRINE_ARCHIVE.md`（コピー→アンカー確認→削除の手順つき）。警告文は
> **その時点の実測で最大節を名指し**する方式へ（`_biggest_sections`）＝**決め打ちの助言は必ず古びる**。
> ⚠️ **アンカーの偽陽性を1件解体**: 金ラボQ8の prose `0.63` は本文に存在せず、§3 の `0.639` への
> 部分一致で通っていた＝§3を退避した瞬間「金ラボが乖離」と**無関係なエラー**を出す地雷（実数値を本文に明記して解消）。
> ⚠️ **派生で非公開研究の流出穴を発見・封鎖**（下表に追加）。**この節はアーカイブ退避しない**＝予算根拠の一次記録。




## 【2026-08-05 退避】(g) 既存記事の法務棚卸し＝黒13件を修正・再発を停止（8/1 15:00-16:20・完了）
...
- 🚨 **最優先だったのは再発停止**＝`auto_weekly_review.py` だけ**コンプラゲートが無く**、Gemini 出力が無審査で
  **毎週月曜07:13に新しい黒を生産**していた（「EURUSD=X …見送ることを強く推奨します」等が公開済み）。
  原因はプロンプトが「来週の取引判断に直接使える内容」を要求していたこと。全面改訂＋**決定論の禁止語ゲート**を追加。
...
- 🔧 **黒13本に35箇所の置換**を適用。ドル円ロングの「強く推奨」＋逆指値160.20円、日経の階段買い
  「¥58,500で1/3…¥53,000で全力」、BTCの「$76K打診→$72K追加→$68Kで本気買い」、トヨタ「押し目買いチャンス」等。
  **VXUS を「海外VIX ETF」と誤記していた事実誤認**（実際は全世界株ETF）も同時に削除。
- 📉 機械スキャン `_compliance_scan_existing.py` の A判定は **141件/51本 → 122件/45本**。
- 📌 **監査官の所見**: 黒13件は全て2026年4〜5月に集中し、**11/13が記事末尾の「対応戦略」テンプレに集中**していた
  ＝「読者に何をさせるかで締める」執筆型そのものが黒の製造機。
  ⚠️ ただし「ゲート導入後の記事に黒は無い」という結論は**8/1夕方に覆った**（月次レポートレーン。auto-memory 参照）
- ⚖️ **弁護士相談アジェンダに6項目追加**（全文は auto-memory `project_marketwatch_compliance`）。最重要＝
  **「不特定多数向け・随時閲覧可能な媒体」除外の射程**（通れば黒の多くは金商法違反でなくなる＝分水嶺）。
- 🟡 **グレー8件は保留**（黒は即修正・グレーは弁護士相談まで）。


## 【2026-08-01 退避③】(h) キオクシア決算記事の公開＋銘柄チャート基盤の新設（8/1 09:00-14:00・完了）

**公開済み** → `guide-kioxia-2026q1-earnings.html`（個別銘柄解説・ライブ実測OK）。5月の解説記事も同時更新。
- Q1実績＝売上1兆7,671億(5.2倍)／営業益1兆2,700億(28倍)／純利益8,422億(46倍)、営業利益率 **13.1%→71.9%**。
  上期予想は純利益36倍で**通期は非開示**。自社株買い上限8,000億・3,000万株、1:3分割（基準日9/30）
- ⚠️ **二次情報は数字が壊れる**。「Q1営業利益 1兆2,700億」と「上期予想から逆算したQ2純利益 1兆2,700億」が
  **偶然同額**だったため各社がラベルを取り違えていた。要約モデルは自社株買いを「8 billion yen」「3000株」とも壊した。
  **個別銘柄の決算は決算短信のパース値（株探等）＋適時開示で照合してから書く**
- 🆕 **`_gen_stock_panel.py`**（引き継ぎのルール表に収録）。5月の記事は「サイト内チャート埋込みに**対応していません**」と
  断って外部リンクを3枚並べていた＝その制約を解消し、当該記述を削除して実チャートに差し替えた
- コンプラ監査は**独立Opusを2回**（初回🟢白・修正後の再確認も🟢白）。指摘は全て検算し3件は本物だった＝
  自社株買い比率の係り先（8,000億は**約3.2%**で5.5%は株数上限）／x軸右端が最終足でない／**決算日の出来高が未確定値**


## 【2026-08-01 退避②】夜の回線輻輳メモ / 7-30〜31 完了リスト

回線輻輳は auto-memory `reference_github_api_ipv4_congestion` に収録済み（引き継ぎからは削除）。
...
## 【2026-08-05 退避】(a) 為替介入＝記事公開で決着（8/3・ライブ確認済み）
...
## 【2026-08-11 退避】ブランドカラー Phase 1 の完了記録（SESSION_HANDOFF より）

退避理由＝SESSION_HANDOFF.md が文書予算30KBを超過したため。**未完の Task 7 Phase 2 は引き継ぎ側に残してある**。
...
**新テンプレ対応**（guide-signal-lab-060 は CSS変数式＝ar(--accent) で既存ルールが1つも当たらなかった。
ライトの --accent と h1 をルール化して修正＋check_guide_draft.py 検査9 で今後を遮断。**誤検知0/295本**）


---

# 📦 2026-09-22 退避（SESSION_HANDOFF.md から）

引き継ぎ文書が 153KB まで肥大し、CLAUDE.md の予算 30KB を大きく超えていたため、**2026-08 以前のセクションをここへ移した**。内容は削っていない（全文そのまま）。
