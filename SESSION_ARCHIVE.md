...
## 📦 2026-07-26 退避（SESSION_HANDOFF.md の7/24以前の完了節）

- **⓪-150年チャート超長期化 7/24深夜✅（オーナー承認済み設計を完遂・ライブ確認まで済）**: charts.html=「150年チャート＋投資史年表」へ拡張完了。①**データ**: `historical-long.json`（7KB・SYNC入り）へ一度だけ焼き込み=`build_historical_long.py`新設（SYNC入り・サニティ8種=日経1989:38915.87/固定360円/金公定段差など全通過）。実ソース=S&P:シラー12月値1871-1984＋Yahoo(^GSPC)1985-2025／日経:FRED 1949-2025／ドル円:360円定数1949-70＋DEXJPUS 1971-2025／金:公定$20.67→$35＋**LBMA公式PM** 1968-2025。⚠️発見3件:(a)**FREDはローカルPythonからTLS遮断**→ブラウザ同一オリジンfetch＋チェックサム突合で`_fred_yearend_cache.json`(ローカル専用)に確保が定石 (b)**Yahoo chart APIは^GSPCを1985年以降しか返さない**（期間指定は黙って切詰め・負エポック不可） (c)ローカルyfinance不調→chart API直叩き。②generate_market_news.py=`load_historical_long`+`merge_historical`（asof_year以前は焼き込みが真実・Actionsは2026年以降だけYahoo接ぎ足し・JSON欠損時は従来1975〜へフォールバック）＋テスト`_test_historical_long.py` 18件PASS。③見せ方=対数スケール必須（🔰説明ボックス付き）・時代帯4色（金本位制/大戦恐慌/ブレトンウッズ/変動相場制・チャートごとに自動クランプ）・プレ1971イベント7件追加（1907恐慌/WWI/1929大暴落/金$35切下げ/ブレトンウッズ/東証再開/朝鮮戦争）・**1602〜1871図解年表**（VOC/チューリップ/南海泡沫/ボタンウッド/ゴールドラッシュ→1878東株）・出典脚注（シラー/FRED/LBMA/Yahoo）。**旧コードの潜在バグ2件も修正**（x軸ラベル全消え=`-01`判定・ツールチップ黒地黒文字）。ローカル検証=構造JS検査＋実描画キャプチャ3枚目視＋モバイル(nav2列/240px)＋ダーク（年表ボックス反転）OK・mw checkエラー0→sync 243件→workflow起動。index側フィーチャーカードと更新履歴も150年表記に更新済み。**残**: ナビ文言の150年化は**オーナー承認済み＝上の🔴在flight項目へ移管**（未着手）。
- **⓪-Claude Design連携 7/24✅**: DesignSyncでMarketWatch部品10種（配色/タイポ/ヘッダー/ナビ/免責/ボックス/カード/表/CTA/チャート様式）を「Design System」プロジェクトへpush済み（詳細=auto-memory `reference_claude_design_project`）。トップページのモック化はオーナー判断で**保留（また後で）**。

- **⓪-図解カタログ 7/24夜✅（オーナー依頼「シグナル発火を図解で・今後の日誌もチャートで」）**: ①**特別記事公開=`guide-signal-anatomy.html`**（「AIはどんな瞬間にシグナルを出すのか——発火条件の図解カタログ」🧪・チャート風SVG3枚=逆張り3種/順張り3種/通知パイプライン・発火条件はエンジンコードから正確に転記・独立Opus監査**🟢白=黒ゼロ修正なし**・グレー2件は既存弁護士アジェンダ①②へ実例添付のみ・ライブ200/guidesカード/SVGはみ出し0/ダーク両対応 確認済み） ②**signal-lab-daily routineプロンプト更新**=SVGのうち1枚「チャート風図解」を必須化（手本=anatomy図1・図2/概念図注記必須/状況次元(env・news等)は背景帯で表現/定点観測日は省略可）＋関連カードにanatomy必須＋**filterキー列挙を16キー現行化（成果戦略・残タスク(iv)消化）**。**→7/25朝06:10の#050が初適用=チャート図解が入るか要確認** ③**過去日誌へ導線注入=全49本完了**=`apply_anatomy_link.py`新設（冪等・meta-line直後にinfo-box・SYNC入り）。⚠️発見2件: (a)日誌はクラウド公開レーン＝ローカルSYNC_FILES外→**Contents APIで一回限り修復push**が定石（7/21ナビ修復と同型） (b)**ローカルの日誌ミラーはGitHubに遅行する**（#047〜#049がローカルに無かった→remote全量突合でpull済み。一括編集の前に必ずAPIでリモート列挙と突合すること） ④**個別チャート図解の後付け=第1弾3本✅（オーナー合意「少しずつ」）**: #041(下降×逆張りレジーム転換)/#048(指数×ロング降格=前半後半の失速図)/#049(押し目反転の入口2系統)に各回専用の概念チャートを注入・はみ出し0/ダーク両対応検査済み・ライブ反映確認済み。ツール=`_inject_lab_charts.py`（ローカル専用・冪等・**次弾はCHARTS dictにSVGを足して--apply→API pushするだけ**）。残り46本から重要回を数本ずつ継続。
...
- **⓪-監視サマリー 7/23夜✅→7/24初運転✅**: `_jp_daily_summary.py`（ローカル/SYNC外・テスト8件PASS）＝毎朝05:45スクリーナー成功後に `JP監視サマリー.html/.txt` をOneDriveへ自動生成（wrapper best-effort）。7/24 06:22初回自動生成を確認済み＝通常運転入り。
- **⓪-Q25Q26 7/23深夜✅**: **Q25=出来高バンド前向き**（登録のみ=queueに📊待機・初判定=前向きN≥600&両端各n≥150）。**Q26=セリクラ実行済み**＝A「大商いの投げ」✅両期間有意だが方向は予想と逆=劣後（セリクラ翌日は買い場でない・左裾破滅的・クラッシュ5〜7倍）／B「閑散の下げ」❌棄却。数値の真実=`_jp_q26_summary.json`・DOCTRINE §2/§3結晶化・Q26退役済み。**同夜に特別記事公開済み=`guide-selling-climax.html`**（機械突合PASS・独立Opus🟢白・ライブ確認済み）。（DOCTRINE declutterは7/24✅＝warning 0）。
...
- **⓪-Q21Q22 7/22夜✅**: Q21=H-V1薄商い説❌非有意／H-V2注目度=逆方向有意（ニュース過熱時は劣後・記事ネタ候補）。Q22罠フラグ=✅両期間合格（クラッシュ約12倍・mw screen⚡列適用済み）。数値の真実=`research/_q21_summary.json`/`_jp_q22_summary.json`・DOCTRINE結晶化・queue_archive退役済み。
- **📌newsレーン30本=ダークモード全欠落**（navだけでなくページ全体にbody.dark無し。テンプレ拡充は別セッションで設計＝7/21ナビCSS節から引き継ぎ・経緯はSESSION_ARCHIVE 7/24退避分）
- **⓪-A 7/19大改修=初運転確認✅（7/20。③降格も7/23実現済み=上の🌅節参照）**。残る予定だけ: ④格言キュー=7/22に#25〜33補充済み→**次の枯渇≒8/5=要補充** ⑤**9/20以降**=15分足セッション追試（research/_fx_session_15m_scan.py再実行・事前登録済み）。
...
<!-- 2026-07-26 declutter: SESSION_HANDOFF.md 予算30KB超過のため退避（完了節）。 -->
## 【2026-07-25 完了】ナビ文言「50年チャート」→「150年チャート」一括更新
...
<!-- 2026-07-26 declutter: SESSION_HANDOFF.md 予算30KB超過のため退避（完了節・第2弾）。 -->
## 【2026-07-25 完了】期限到来の確認2件（#050のチャート風図解 / REVIEW.mdの🚩）
...
<!-- 2026-07-26 declutter: SESSION_HANDOFF.md 予算30KB超過のため退避（完了節・第3弾）。 -->
## 【2026-07-25 完了】レジーム転換検知オラクル＝稼働
...
<!-- 2026-07-26 declutter: Q29 の設計/実行節に引き継いだため退避。 -->
## 【2026-07-26】PEAD Q28＝検定不能でクローズ（当日の詳細記録）
...
<!-- 2026-07-26 declutter: Q29実行節が上位互換のため退避。 -->
## 【2026-07-26】Q29 の設計（検出力を構造だけで確定した手順）
...
## 【2026-07-27 退避】SESSION_HANDOFF から移した 7/26 完了9節
...
## 2026-07-27 退避（SESSION_HANDOFF の 7/27 完了節6本）

> 退避理由＝HANDOFF が予算30KBを大きく超過。全て決着済みで、結論は DOCTRINE §0-11/§0-12 と
> `research/hypothesis_queue_archive.md` に結晶化済み。ここは経緯の保存用。

- **⓪-💰Q42/Q43 コスト構造＝「攻め」から「コスト」へ問題を移した2本（7/27・全文は `hypothesis_queue_archive.md`）**
  - **発端**＝オーナー「攻めの検証は全滅。どうしたら期待値でプラスになるのか。アイデアが枯渇した」→ **26本の仮説はすべて『どのシグナルが勝つか』を訊いており、『1回のトレードがいくらかかるか』を訊いたものが1本も無い**と気づいた。
  - **Q42（20年BT・3足共通のFX4銘柄・N=15,225）＝❌棄却だが手がかり**: 時間足×期間の交絡を**母集団の限定で構造的に除去**（Q34/Q35で見つけた罠の再適用）。1h グロス+0.0252/コスト0.0531R/ネット−0.0279/**倍率0.71**／**4h +0.0486/0.0264R/+0.0222/倍率2.77**／1d **−0.1256**(勝率36.9%)/0.0094R/−0.1350。4h vs 1h 差+0.0501 **p=0.083**・両期間下限マイナス＝**手がかり止まり**。
  - **Q43（ライブ・両レーン共通18銘柄・N=1,881）＝❌棄却＝ライブでは再現しない**: 1h グロス−0.0093/ネット−0.0757/**倍率−0.21**／4h グロス**−0.0169**/ネット−0.0485/**倍率−0.81**。差+0.0273 **p=0.659**。**🔑ライブでは両レーンともグロスが負＝倍率がマイナス＝コストをいくら下げてもプラスにならない**。4hが有利に見えるのは**コストが安いからだけでグロスはむしろ悪い**＝Q42と機序が違うので「両方一致」とは読めない（§0-6）。⚠️登録時に**出目の既見を明記**した非盲検テスト（Q16の前例）。
  - **🔑 副産物が本体を上回った（探索・判定ではない）**＝2レバーの組み合わせ実測。**現状のネットは −0.0605 CI[−0.120,−0.001]＝全域マイナス**（統計的に検出できる負け）→ **守り6ゲートでグロスが −0.0106→+0.0241 に転換** → **さらに1hに絞ると 勝率44.6%・グロス+0.0400・ネット−0.0112**＝**損益分岐43%を勝率が上回り、残差は0.0112Rのみ**。⚠️ゲート後に4hへ絞ると**悪化**する（グロス+0.0034）＝4h案は完全に否定。
  - **📌 結論＝問題が「攻めのエッジ探し」から「コスト22%削減」に変わった**。グロスは既に足りている。**Q44として事前登録＋前向き検証が必要**（現状はCI[−0.125,+0.102]・68日・N617の探索）。並行して**実務側**＝業者の実スプレッド取得・高スプレッド銘柄（`SI=F` 0.08%／`CL=F` 0.05%）の除外。
  - 成果物＝`_q42_cost_timeframe.py`／`_q43_live_timeframe.py`／各summary.json。**コストは必ず既存の `signal_lab_sweep.cost_r_of` を import**（7/27に自作フラットモデルで誤った経緯あり）。
...
## 【2026-07-28 退避】7/26 完了節3本（SESSION_HANDOFF.md より）

> 退避時に、本文へ埋もれていた未解決2件を SESSION_HANDOFF の宿題 #7 / #8 へ昇格させた
> （①🔴黒の非公開受け皿が未実装 ②weekly-trade-review が MY_TRADING_RULES の旧版を読む）。
...
## 📦 2026-07-30 退避（7/27〜7/28 のセッション記録）
