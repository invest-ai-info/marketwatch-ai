# 🔖 セッション引き継ぎ（最終更新: 2026-08-12 07:00）
...
> 🔴 **止まって見えたのは表示が古かっただけ**。Q7/Q16 のトラッカーは**手動実行設計**で1か月誰も回さず、`mw evolve` は
> Q16 を「N 0/100」と表示し続けていた。実際に回すと **N=27/100・14件/週・初判定9月上旬**＝研究は進んでいた（危うく削除候補にするところだった）。
> 恒久対策＝タスク **`MarketWatch_Research_Trackers`（毎日07:40・`_research_trackers_daily.py`）**を新設・疎通確認済。**前向き観察6本は全て健在**。
> - **退避2件**＝Q10（同日対照が構造的に不成立＝使える日数0/661）／Q27（着手条件が7/25から機械判定で未達＋Q34❌で期待値低下が明記済み）。archive＋登録簿 `archived`。
> - ⚠️ **Qブロックを消すと前後の境界が動いてハッシュが壊れる**（`---` の帰属が変わる）。Q44・Q25 で2回踏んだ＝退避時は前後の区切り行を元の形に戻す。
