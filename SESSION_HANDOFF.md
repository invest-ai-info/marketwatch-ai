...
> - **Q10（及川H-O4）＝対照が作れない**。データは足りる（1h×19銘柄・train 718/holdout 1118・L=34でtrain 21ブロック）が、
>   標準の主対照＝同日クロスセクション `min_ctl=30` で使える日数 **0/661**（同日対照は中央16・最大18＝**19銘柄の理論上限**）。
>   下げれば Q29 の罠。加えて 対照/主要評価指標/合格基準/検出力 が未記入（7/26必須欄化より前の登録）＝**合否基準が無い**。⇒ **状態 ⏳→💬**
> - **Q24（注目度エッジ）＝入力が存在しない**。BTログ2本（21,291件・16,040件）とも **news_count 保有率 0.0%**
>   ＝news は発火時にしか記録できず遡及復元不可（sweep実測でも195仮説中 news= ゼロ）。時間分割もライブ73日分しかなく、
>   その窓は Q21 H-V2 の**発見窓そのもの**＝覗き見。⇒ 残る一手は**前向きtracker登録のみ**＝**オーナー判断待ち**（変種9本 or 主張1本／チェックポイントN）。**状態 ⏳→💬**
> - **Q27（レジーム条件付き）＝着手条件が未達**。`_regime_targets.py` の抽出は 7/25 と同じ**3件のまま**＝条件①不成立、条件②の判断も無し。
>   加えて Q34❌（§0-11）が「Q27の期待値も同時に低下」と明記＝**条件が揃っても事前確率が下がっている**。⇒ 見送り継続（⏳のまま）。
> ⚠️ **副産物＝第4類型の実例を1件捕獲**: ローカル `signals-log.json` は **1,028件(6/18止まり)** に対しリモート **3,079件(7/31)**＝
> **33%・6週間遅れ**。逆に `signals-log-backtest*.json` は**リモート404＝ローカルが真実**。**入力ごとに真実の所在が逆転する**ので毎回突合する。
> 💡 **恒久対策の候補（未実装）**＝必須欄チェックは「欄が埋まっているか」しか見ておらず、**「埋めた設計が手元のデータで実際に走るか」を見ていない**。
> Q10・Q24 はどちらもそこで詰まった。登録時に1回だけ実行可能性を実測する検査を足せば同型を防げる。
>
> ## 🧹 (f) 不要物の棚卸し＝**ほぼ何も無かった**（7/31 22:30）
> ジョブ系は**休眠ゼロ**（Actions 19本・Windowsタスク9本とも稼働／月次2本の「28日前」は1〜3日実行で正常）。
> ファイルの孤立は12件・**計34KB のみ**で、残り76MBは全て参照あり。実施＝**11件21KB削除**（再生成ログ3件＋6月の
> 使い捨てスクリプト8件。`_q30_test.py` は Q30棄却の再現手段なので保持）／**バックログの死に4行を退避**
> （EAP=Q31棄却済・COT=Q30棄却済・PEAD=Q29クローズ済・粗利益率=恒久不可）＝**3行が「着手可」と書いたまま実行済み
> ＝在庫の嘘**だった。research/ 124→114ファイル。
> 🚨 **やらなかったこと＝価格キャッシュ50MBの削除。「再取得可能」という前提が実測で否定された。**
> Yahoo の遡及上限は **1h=730日 / 15m=60日**で、`_all1h_prices.json` は **329日ぶん**、`_fx1h_prices.json` は
> **296日ぶん**が**二度と取れない**（`_all15m_prices.json` は取得スクリプトすら無い）。
> **「取得スクリプトがある＝再取得可能」は誤り。API の遡及上限で判定する。**
> ⚠️ 別件で1つ検出＝`MarketWatch_JP_OpenLog` が 7/31 12:01 に **267014（タスク強制終了）**で落ちた＝7/23 の永久欠測と同じ型。
> 7/31 に入れた**直近5営業日巡回バックフィル**が 8/3 の次回実行で効くはず。**8/3 に取りこぼしが埋まったか確認する**。
