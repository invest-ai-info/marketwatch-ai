...
> - **(d)** DOCTRINE 予算を 30/36KB へ改定（オーナー承認・根拠は `_doctrine_check.py` の `DOCTRINE_WARN_KB` 直上コメント）
> - **(e)** 仮説3本（Q10/Q24/Q27）は**走らせる前に設計が成立しないと判明**して全て未実行。恒久対策＝`_precheck_feasibility.py`。
>   **Q24 の前向きtracker登録だけがオーナー判断待ち**（変種9本 or 主張1本／チェックポイントN）
> - **(f)** 不要物の棚卸し＝ほぼ何も無かった（11件21KB削除）。⚠️**価格キャッシュは消さない**＝Yahooの遡及上限
>   （1h=730日/15m=60日）を超える分は**再取得不能**（`_all1h` は329日ぶん）
...
>
> **(c) 日本株の蓄積＝結論は秋**（詳細は auto-memory `project_jp_open_edge`）。23営業日/1,053行・カバー率95.8%。
> 日数ゲート(≥20)は達成だが各群N≥100は H2/H5 の2つだけ＝本検定は事前登録どおり自動見送り中。
> **ゲート到達は10月中旬目安**（0.5%差の検出に78営業日・旧「8月上旬」は誤りで訂正済み）。人がやる作業はゼロ。
> 7/23の永久欠測は `jp_open_logger.py` を直近5営業日巡回へ修正して解決＝**タスク設定でなくコード側で吸収するのが正しい**。
>
