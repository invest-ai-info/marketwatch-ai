...
## 🚨 2026-08-29 夜: 事前登録簿の改竄検出が Q45 で鳴っている（**私の変更より前から**）
...
- **Q45 の事前登録違反**は**私の変更より前から存在**（Q47 追加後も同じ1件のみ）。`hypothesis_queue.md` は SYNC外・git外・`.bak` なし＝**リポジトリからは復元不能**。推奨＝OneDrive版履歴を確認 →（無ければ）**Q45を無効化して新Q番号で再登録**。⚠️**登録簿の書き換えは絶対にしない**（オーナー判断待ち）
