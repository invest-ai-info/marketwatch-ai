...
- **⓪-D 進化ループ続き**：Q3〜Q6は7/8消化済み。次候補=**Q8金ラボ第2R**／**Q11ズールー前向きトラッカー設計**（fins月次更新の整備が先）／**Q12グレアム・バフェットp値再監査**（ブロックboot再検定・効果量が残るか）／Q10及川H-O4（低優先）。TT3前向きは月1回`_jp_turtle_tracker.py`。
- **⓪-E 🆕巨匠シリーズ「今風アレンジ枠」＝オーナー合意（7/8夜「そのまま続けて+アレンジで生かせないか」→時間切れで明日に持ち越し・着手候補は未選択）**。原則=原型の見た目でなく再現した構造（守り/逆張り/安さ）に接ぎ木・1アレンジ=1事前登録・新標準で検証。候補4本:
  **A**=資産クラス版デュアルモメンタム（日経/S&P500/金/米債の月次乗り換え・原典回帰・Yahooで20年・記事#6有力候補）／
  **B(推奨)**=深押しナイフ防御ゲート（グランビルG4乖離-20%とQ3三空を「回避条件」に反転しblowup回避を検証→合格ならmw screen危険列・パニック反発の除外フィルタ）／
  **C**=ズールー前向き実装（=Q11+screener参考列）／**D**=グレアム防御×PEG成長の複合1本（複合は前科あり=期待控えめ明記）。
  ※「すでに生きているアレンジ」の整理も記事ネタ（赤三兵→連騰≥4危険ゲート・タートル資金管理→ATRベースSL/2%ルール＝原型のエントリーは死んでも部品は生きる）。
