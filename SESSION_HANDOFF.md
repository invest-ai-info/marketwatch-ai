...
### 次の一手（オーナー判断待ち）
1. ✅完了：Track 2 を対プール差分へ正式化＋危険指標を「ブローアップ率＋勝率＋MAE分位＋対プール差分」に正式化。次=続伸合成「売られすぎ反発×大型防御×非過熱」を事前宣言1本で確認＋危険フィルタを回避ゲート宣言。
2. Track 2 前向きholdout＋生存者バイアス補正（上場廃止銘柄）＋ATR正規化SL/TP感度。
3. Track 1 ブロックブートストラップ＋取引コスト感度＋ポートフォリオ相関＋ライブforward R接続。
4. いずれかを「🧪AIシグナル研究日誌」記事化（透明性戦略）。公開は人間＋compliance(Opus)監査経由・自動公開しない。
