...
>
> ## 💾 (h) キオクシア決算記事を公開＋銘柄チャート基盤を新設（8/1 09:00-14:00）
...
> - 🆕 **`_gen_stock_panel.py`**（下表）。5月の記事は「サイト内チャート埋込みに**対応していません**」と断って
>   外部リンクを3枚並べていた＝その制約を解消し、当該記述を削除して実チャートに差し替えた
...
| **自動生成レーンの投資助言化を防止** 🆕8/1 | `auto_weekly_review.py` の `FORBIDDEN_TERMS`＋`forbidden_terms_in()`（テスト=**`_test_weekly_review_gate.py` 18件**） | このレーンだけ Opus ゲートが無く**毎週月曜に黒を生産していた**。禁止語に触れた生成文は不採用→次モデル→全滅なら決定論テンプレへ。**LLMの自己申告に頼らずコードで止める**（プロンプト修正だけでは再発する） |
