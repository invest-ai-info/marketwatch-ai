...
- **⓪-🧹declutter＝実施・ただし予算の見直しが要る（7/26 16:05）**
  - Q30/Q31 を `hypothesis_queue_archive.md` へ退避（登録簿に `archived` 追記）。**キュー 38.3→27.0KB**。ハッシュ違反ゼロ・KPIも20本/生存8/棄却12のまま維持。DOCTRINE も自分が足した2行を圧縮して 25.2→25.0KB。
  - **⚠️ ここからは私では減らせない**: 閉じた仮説（Q28/Q30/Q31）は全部退避済みで、残り9件はすべてアクティブ。内訳＝Q29 6.7KB・Q27 4.3KB・他7件は平均1.3KB。**27.0KB が今の記録水準での自然なサイズ**であり、14KB閾値は「1件1.3KB」時代の設定。
...
- **⓪-❌Q31 EAP 決算公表前ドリフト＝棄却（7/26 15:53・棚卸しで解禁した4本のうち2本目）**
  - **リターン差は再現した**: K=5/10/20営業日の3本とも**両期間で同符号・有意**（train +0.206/+0.421/+0.616% ／ holdout +0.451/+0.587/+0.924%・BH-FDR後 q 0.0003〜0.0087）。**窓が長いほど差が大きい用量反応**も揃った。
  - **落ちたのは blowup**（MAE<-20%の割合）。train は3本ともシグナル側が低いのに **holdout で逆転**（K10で 0.4%→3.8% vs 対照 1.1%→1.8%）＝**発表前の上乗せは直近期間でテールリスク増を伴う**。事前登録の基準どおり **合格0/3**。
  - **緩めなかった**。blowup は登録済みの合格基準。「リターンだけ見れば合格」に書き換えるのは結果を見た後の基準変更＝§0-3の罠。**§2（防御）側の知見として残す**ほうがこのプロジェクトの実績と整合する。
  - **先読み防止**: 前年同期の実績+364日で予想発表日を作る（実績日は信号に使わない）。予測精度＝中央値誤差+1日・±5日以内89.1%。「発表前」の窓に実発表が紛れる混入率は6.0〜6.7%（リターン未参照で実測）。
  - **⚠️ 検出力の見積もりをQ29に続き2度目の失敗**（種類は同じ）。推定は「窓が**覆う**日数」1,007日→17/17ブロックだったが、`samedate_diff_p` が実際に数えるのは「窓が**始まる**日」546日で、K=20は train 7ブロック＝検定不能だった。**教訓＝見積もり器は「検定が実際に数える量」を計算すること。次回は推定と実測を同じ関数で共有する。**
  - 成果物: `research/_q31_test.py` / `_jp_trading_calendar.json`（1,237営業日）。DOCTRINE §3 に記録済み。
