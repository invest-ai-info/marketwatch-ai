...
> ## 🌅 次セッションの入口
>
> ## 🔴🔴 **最優先: ブランドカラー適用が「途中で止まっている」**（2026-08-05 03:17 時点）
> **ライブサイトは今、新色ページと旧色ページが混在している。**破損ではなく「送り切れていない」だけ。
> **やることは sync の再実行だけ**（冪等＝送信済みはハッシュ比較でスキップ）。
>
> **止まった理由＝夜間の回線輻輳。** `api.github.com` は ping 6ms で通るのに **TCP:443 が3回連続20秒タイムアウト**した
> （02:50頃）。1回目の sync は **成功189 / 失敗68**。**⏰ 03:20 に TCP443 が復旧を確認**＝そのまま手順1から流してよい。
> ⚠️ ただし**また落ちる可能性がある**ので、必ず先に `Test-NetConnection` で実測すること
> （auto-memory `reference_github_api_ipv4_congestion`＝日による）。
>
> ### 復旧手順（この順で）
> ```
> 1. Test-NetConnection api.github.com -Port 443     # 繋がるか先に実測
> 2. python _pull_mirror.py                          # dry-run。古い/欠落があれば --apply
> 3. PYTHONUTF8=1 python sync_to_github.py           # 残りを送る（時間がかかる。背景実行推奨）
> 4. クラウドレーン記事の push（下記「未完の作業」）
> 5. python mw.py trigger update-market-news.yml     # 6コア再生成
> 6. ライブ目視（PC幅・モバイル幅）→ オーナーへ Phase 1 の確認を依頼
> ```
...
> 計画＝`docs/superpowers/plans/2026-08-04-brand-color.md`。ツール＝`apply_brand_color.py`（テスト39件）。
> ローカルは**305ファイル・1734箇所に適用済み**・冪等・`mw check` エラーゼロ。
>
...
> - **クラウド記事を触る前に必ず `_pull_mirror.py --apply`。** 実測で **135本が古い/欠落**だった（やらずに push すると120本を巻き戻す）
> - 山吹の適用先「記事カードの左罫」は**実在しなかった**＝Phase 1 の山吹はロゴのみ（無理に適用先を作らない）
>
> ### 未完の作業
> | # | 内容 |
> |---|---|
> | 1 | **sync の残り**（上記手順）。これが最優先 |
> | 2 | **クラウドレーン記事の push**。SYNC_FILES 非登録の約160本。恒久登録は設計思想と衝突するので**一度だけ Contents API で送る**方針（`sync_to_github.upload_file` を流用） |
> | 3 | **Task 6: ロゴ**。`apply_logo.py` の SVG 定数を 地`#1E3A6E`・丸`#E8A317` へ（現行の赤丸 `#d92b2b` は意味色と衝突）。⚠️PNG/ICO は再生成されない可能性あり＝要確認 |
> | 4 | **Task 7: Phase 2**（本文リンクを藍(明)＋**下線**・オーナー承認済み）。`--phase 2` は実装済み・dry-run で136本403箇所 |
> | 5 | **Task 8: 記録**（CLAUDE.md の保守ツール表とこの表に1行ずつ） |
>
> ## ✅ **(a) 為替介入＝記事公開で決着**（8/3・ライブ確認済み）
> `guide-jpy-coordinated-intervention-2026-08.html`。日米協調は**報道ベース・当局未確認**の前提で執筆。
> **「15年ぶり(2011年G7＝円売り方向)」と「28年ぶり(円買いでの日米協調＝1998/6/17)」は別物**＝報道が混同しがち。
> 外貨準備は財務省一次資料と突合済（総額 1,287,476百万ドル・6月末）。介入実績 6/29〜7/29 は **0円**＝7/30 が起点・実額は8月末公表。
> 1998/6/17 の 2,312億円は**財務省の日次CSV(1991年4月〜)で検証**＝同四半期 4/9 1,957＋4/10 26,201＋6/17 2,312＝30,470 で一致。
> ⚠️ 独立Opus監査を3巡し**私の誤りを4件**捕捉（材料リストが円安側に傾く／**n=7の標本に協調介入が1件も無いのに外挿**／
> 修正時に断定を復活／検証パーサが行を落とす）。**外挿の適用限界を🔴で自己申告する型**は速報記事へ横展開する価値あり。
