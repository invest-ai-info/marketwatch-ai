# 🔖 セッション引き継ぎ（最終更新: 2026-08-19 22:35）
...
| # | 内容 | 影響 |
|---|---|---|
| A | **騰落レシオが別の指標**。一次ソース（nikkei225jp.com）の正規表現が現在のページ構造と一致せず（実測191,448bytes取得・抽出失敗）、フォールバックの「TOPIX ETF 1306.T の値上がり日数÷値下がり日数」に落ちている。ライブの177.8%は25営業日で16勝9敗＝`16/9*100` に一致 | **説明文は「東証プライム全銘柄」「リアルタイム更新」と書いている**＝別物を本来の名前で出している |
| B | **vix.html の警戒ライン・恐怖ラインの点線が描かれていない**。annotation の定義はあるのに `chartjs-plugin-annotation` を読み込んでいない | 見た目だけ。1行追加で直る |
| C | **ダークモードが5ページで機能しない**（vix / charts / hot-assets / calendar / market-health）。ダークCSSを注入するJSが `body.dark` セレクタの有無で「明示ルールあり」を判定するが、`site-search.js` の `body.dark #ss-btn` に引っかかって**注入をスキップ**している。ライブで背景が白のままなのを実測 | 自前ルールを持つ index / guides は正常。**5ページはトグルしても何も変わらない** |
