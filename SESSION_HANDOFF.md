...
> - 🔑 **🚫stale で `--force` を反射で打つな。正順＝取り込み → 自分の変換を乗せ直す → 「差分は自分の変換だけ」を機械照合 → `--force`。**
>   8/5 は42本中**39本でガードが正しかった**（スマホCSS37本・更新履歴・新記事カード63行）。8/11 のナビ展開でも同じ形で
>   クラウドの新記事7本を救った＝**この手順は2回とも当たっている**（全文は SESSION_ARCHIVE【2026-08-08 退避】）
> - 山吹の適用先「記事カードの左罫」は**実在しなかった**（無理に適用先を作らない）
>
> ### 未完の作業
> | # | 内容 |
> |---|---|
...
>
> ✅ **8/5 完了分**（ヘッダーラベル197本／Task 6 ロゴ＋`make_favicons.py`／新テンプレ `var(--accent)` 対応）は
> SESSION_ARCHIVE【2026-08-11 退避】へ。**未完は上の Task 7 Phase 2 のみ**。
>
> 🔑 **適用漏れはライブの computed 値で確かめるのが最短**（ローカルの grep では「役割が違う同じ色」と区別できない）。
>
> 🔴 **8/5 の最大の教訓＝「壊れたのにアラートが鳴らない」経路を疑え。**
> 手順書に書いた文字で Pages ビルドが12時間全失敗したのに、ライブは古いビルドを配信し続け `health-check` は緑のままだった
> （監視が"日付が今日か"しか見ていない）。再発防止は下の強制ルール表（Liquid検査／一括コミット）が単一の真実。
>
...
>
> 🔴 **オーナー判断待ち＝`hypothesis_queue.md` 40.2KB（予算36KB）の予算改定**。2026-08-11 に Q10/Q27 を退避し 45.1→40.2KB まで削ったが、
> **残る8本（Q7/Q11/Q16/Q23/Q24/Q25/Q44/Q45）は全て前向き観察中か登録直後＝退避できるものが無い**。実測と根拠＝`_q45_design.md` §6。
