# 🔖 セッション引き継ぎ（最終更新: 2026-08-19 11:30）

## 🚩 次のセッションは、ここから（状態はすべて 2026-08-19 11:30 JST の実測）

### ① 最初にやること — 🔴 格言キューが**今日で枯渇**した（明日止まる）

**#37〜#39 は全部公開済み**（`tooku-wa-sakeyo` / `rekishi-wa-in-wo-fumu` / `shinbo-suru-ki` の3本ともライブ200を実測）。
つまり **`drafts/PROVERB_GUIDE.md` のキューは残ゼロ**＝**明日 8/20 10:12 の routine は書くものが無い**。
補充するか、格言レーンを畳んで詐欺レーンに寄せるかの判断が要る（8/13 に一度枯渇した時は #34〜39 を補充した）。

🔑 **キューの ✅公開済 マークは当てにならない**＝proverb/scam とも **routine はマークを書かない**。
実測では remote の `PROVERB_GUIDE.md` は #33〜39 が全部「未公開」表示のままだった。
**数えるときは公開ファイルの実在で数える**（`guide-proverb-<slug>.html` が live 200 か）。

📌 詐欺レーンは #5 まで公開済みで **#6〜#10 の5本**が残っている（こちらはまだ余裕）。

### ② 今日〜明日に見るもの

- **automation-health ⑧ が鳴り続ける可能性**＝10:58 の回は「book-watch-weekly（4日放置）/ signal-lab-072（1日放置）」と出たが、**実体はどちらも解決済み**
  （book-watch＝8/18 に新刊2冊を公開／#072＝8/19 11:0x に公開・ライブ200）。#072 は次回から自動で消える。
- ⚠️ **book-watch だけは自動で消えない見込み**＝⑧は `guide-<対象>.html` の実在で解決を判定するが、
  REVIEW.md の見出しは対象が `book-watch-weekly`。**実体は `guide-new-books.html`** なので永久に一致しない。
  → 8/18 に signal-lab で同型を直した `normalize_target()`（`check_automation_health.py`）に
  **book-watch → guide-new-books の対応を1行足せば直る**。診断は済み・未着手。
  ⚠️ 足したら**必ず過去全件で誤検知を測ってから**入れること（下の教訓）。
...
| **格言レーンをどうするか** | キュー残ゼロ。①補充する ②畳んで詐欺レーンに一本化 ③別シリーズへ——の判断（①は過去2回やっている） |
| **Issue #4 を閉じてよいか** | 中身（#065/#067）は解決済み。8/19 時点で残る指摘は上の②だけ |
