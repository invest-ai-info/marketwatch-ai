...
| **やりかけの仮説を増やしすぎない（WIP上限）** 🆕8/11 | `_doctrine_check.py` の `QUEUE_WARN_N/QUEUE_ERR_N`＝**12本で黄信号・16本で error**（`check_queue_wip`・テスト10件） | 旧・バイト予算(36KB)を置換。**較正が1日ずれていた**＝36KBは「1.3KB/本」前提だが、必須欄等の証拠要件を入れた 7/26 **以降**のブロックは平均7.8KB（Q44 6.5・Q45 9.1）／以前は平均1.6KB＝**証拠を厚く書くほど罰せられる**逆向きの予算だった。加えて①ヘッダ13.3KBが固定費で実効枠は約3本 ②キューは毎セッション自動注入されない＝バイト数はオーバーヘッドの代理にならない ③サイズが本当に代理していたのは**同時進行の本数**（8/11 に前向き6本中2本の1か月放置を実測）。error は**1本閉じるまで新規登録しない**WIP上限。⚠️`declutter_audit.py` の表からキューを外した（二次側が「文書を削れ」という一次側と食い違う対処を出すため） |
