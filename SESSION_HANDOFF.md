...
- **🔴⓪-A 最優先＝7/7夜に入れた自動化3件の「実環境・初運転」を確認**（下記は全て今夜が初の朝実行＝未実証）:
  (1)**jp_dailyフェッチ修正**（`incremental_fetch`に壁時計バジェット720s＋タスク制限30→45分）＝今朝は
     ネット未接続でフェッチがハング→30分killされカード未生成だった。**明朝カードが生成されるか**（`_jp_heartbeat.txt`が7/8か）
  (2)**レイク自動補充** `MarketWatch_JQ_Lake`(05:30)＝`_jq_daily_lake`末尾が7/7になっているか
  (3)**スクリーナーCSV自動生成** `MarketWatch_JQ_Screen`(05:45)＝`_jp_screen_latest.csv`が7/8更新・`_jp_screen_heartbeat.txt`=7/8か
  **確認コマンド**＝`python -X utf8 _jp_health_check.py`（4項目：寄り付き/レイク/スクリーナー/カードが全✅なら成功）。
  **アラートtxtの有無**＝作業フォルダに`JP*_要確認.txt`が無ければOK。**コケていたら**各txtの指示どおり手動実行＋原因診断。
- **⓪-B 7/8(水)朝の既存無人運転**：(1)08:40 autopublish が**⑰インフレ(inflation-real-return)を公開するか** (2)⑱以降のキュー消化継続。
- **⓪-C idea-scout-weekly**：初回手動E2Eは7/7済（jp-asset-growth/max-lottery-effect/jp-turn-of-monthの3件がidea-inboxに）。**初の定時運転=7/12(日)14:00 JST**→翌セッションで転記（転記slugは idea-tested-slugs.txt へ）。
- **⓪-D 進化ループ続き**：次候補=**Q3 酒田五法**（パターン系・実装前に三空/赤三兵等の数式定義を補遺で確定）。以降Q4グランビル/Q5デュアルモメンタム/Q6ズールー。TT3前向きは月1回`_jp_turtle_tracker.py`。
