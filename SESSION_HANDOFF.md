...
- **🖼️ シリーズ標準＝インラインSVG概念図（2026-06-02 確立）**：チャート系記事は文字だけだと伝わらないので、**手描きSVGの概念図**を入れる方針に決定。一目均衡表に3図実装済（①全体像＝価格+転換線+基準線+雲+遅行スパン、②雲とローソク足の位置関係3パネル、③三役好転の3条件）。`<style>`に `.chart-figure/.svg-panels/.s-*` クラス（**ライト/ダーク両対応**・実価格不使用＝コンプラ安全）を定義。**実在価格でなく概念図とキャプション明記**。次弾以降も同手法で図解する（MACD=2線+ヒストグラム、RSI=0-100オシレーター等）。ローカル確認は `.claude/launch.json` の `marketwatch-static`（python http.server 8765）→ preview スクショ。
- **🎯 次セッション：シリーズを継続して量産**（第3弾以降：MACD／RSI／ボリンジャーバンド／フィボナッチ（実装済み fib_pullback と連携）／エリオット波動／出来高 …。**各記事にSVG概念図を必ず添える**）。狙い＝**エバーグリーン×高検索需要×低コンプラ＝SEO・AdSense両方に効く**。**内部リンクで束ねてトピック権威性**を作る。「薄い量産はNG・質が命」。
