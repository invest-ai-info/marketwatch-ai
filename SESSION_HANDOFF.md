...
> - 🆕 **生成スクリプトの CSS は f-string ＝波括弧が `{{`。** 素の形だけで名指しすると**生成ページだけ31箇所が素通り**する
...
>
> 🔑 **Phase 1 で二度踏んだ構図＝「名指しの形が実体と違う」**。①CSSブロックだけ名指し→f-string の `{{` を取りこぼし ②CSSルールだけ名指し→**要素の `style` 属性**を取りこぼし。
