...
> 2. 🚫 _pull_mirror.py --apply を実行しない（下記の理由）。差分の正体を先に測る
> 3. PYTHONUTF8=1 python sync_to_github.py           # 残りを送る（時間がかかる。背景実行推奨）
> 4. 🚫 で止まったら reconcile（下記）→ 色を乗せ直す → --force で再実行
> 5. クラウドレーン記事の push（下記「未完の作業」）
> 6. python mw.py trigger update-market-news.yml     # 6コア再生成
> 7. ライブ目視（PC幅・モバイル幅）→ オーナーへ Phase 1 の確認を依頼
> ```
>
> ⚠️⚠️ **2026-08-05 訂正＝この手順の旧2番は罠だった。**
> `_pull_mirror.py` は「クラウドレーンはリモートが真実」という前提で**ローカルを無条件に上書きする**。
> だが今は**ローカルに未 push のブランド色が乗っている**＝前提が崩れている。実測（blob sha 全件照合）:
> **174本の差分の正体は 163本がブランド色のみ・11本がブランド色＋行末(CRLF/LF)・クラウド側の新内容は0本。**
> つまり `--apply` は**163本の色を消すだけ**の操作だった。7/31 時点では正しい助言、いまは逆。
> **判定法＝ローカルから `apply_rules(..., revert=True)` して remote の blob sha と一致するか**（一致＝差分は色だけ＝pull不要）。
>
> ⚠️ **逆に `--force` も反射で打たない。** 8/5 の sync で 42本が 🚫 になり、中身で検証したら **39本はガードが正しかった**＝
> クラウド側に `fix_mobile_overflow.py` のスマホ最適化CSS/JS(37本)・`generate_market_news.py` の更新履歴7件・
> `guides.html` の新記事カード63行が在った。`--force` は**それを全部巻き戻す**ところだった。
> **正しい順序＝リモート取り込み → `apply_brand_color.py --apply` で色を乗せ直す → 本物の差分ゼロを再検証 → `--force`。**
...
> - ~~**クラウド記事を触る前に必ず `_pull_mirror.py --apply`**~~ → **8/5 撤回。**ローカルに未 push の変換が乗っている間は
>   このツールが**その変換を消す**（実測163本）。ミラー遅行の解消と「未 push の作業を守る」は両立しない＝
>   **差分の正体を revert 照合で測ってから決める**。無条件に走らせる手順は書かない
