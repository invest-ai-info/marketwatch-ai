...
### 🔜 次セッションで最初に確認（在flight・2026-07-24夜更新）

- **⓪-🔜次セッション最優先=50年チャート超長期化（オーナー承認済み・7/24夜・設計合意済み）**: charts.htmlを「150年チャート＋投資史年表」へ拡張。①**データ**: S&P系=シラー月次1871〜（Yale公開）／日経=FRED NIKKEI225 1949〜（キー不要CSV）／ドル円=FRED DEXJPUS 1971〜＋固定360円(1949-71)は史実定数の水平線／金=1968〜市場価格＋公定価格の段差($20.67→$35)。**過去分は静的`historical-long.json`へ一度だけ焼き込み**（ローカルスクリプト新設・SYNC入り）→Actionsは従来どおり直近だけYahooで接ぎ足し（古いデータを毎日再取得しない） ②**見せ方**: 対数スケール必須・時代背景帯（金本位制/ブレトンウッズ/変動相場制）・イベントマーカーをチャート上へ＋既存イベント表と連動・**1602〜1870は図解年表セクション**（チューリップ/南海泡沫等＝連続価格データは存在しないため年表で語る正直設計） ③**実装**: `generate_market_news.py`のcharts.htmlビルダー拡張（6コア=HTML直編集禁止）・出典脚注（シラー/FRED）明記・モバイル/ダーク確認→sync→workflow→ライブ確認。
