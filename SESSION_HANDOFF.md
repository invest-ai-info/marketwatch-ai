...
### 🔜 次セッションで最初に確認（在flight・2026-07-25朝更新）

- **⓪-🔴在flight＝ナビ文言「📈 50年チャート」→「📈 150年チャート」一括更新（7/25 18:28更新・ローカル完了／リモート反映は api.github.com 到達不能で中断）**
  - **進捗**: ローカル置換＝**完了・検証済み**（`unify_navbar.py` 25行目を150年へ→`--apply`で guide-*.html 215本／生成スクリプト8本＋静的HTML5本＝25箇所／`guide-nikkei-vs-topix`・`guide-us-china-summit-2026-05` の関連カード・本文リンク3箇所）。**`mw check` エラー0**。
  - **⚠️ ライブは混在状態**（7/25 18:20実測・raw経由）: 旧ラベル保持244ファイル中 **✅75反映済み／⏳169未反映**。内訳＝①sync対象で未反映**51**（通信断で落ちた分・`guides.html`含む）②Actions再生成待ち8 ③Contents API未実施**110**。
  - **中断理由**: `api.github.com` が **TCP443 到達不能**（IP 20.27.177.116・AAAAレコード無しでIPv6迂回も不可）。`github.com`／`raw.githubusercontent.com`／ライブサイトは正常。sync残り・Contents API・workflow trigger は**すべてこのホスト経由**のため続行不可。sync 2回＋単発GET1回で失敗＝CLAUDE.mdのリトライ上限に従い中断。
  - **復旧後の再開手順（3コマンド）**: ①`PYTHONUTF8=1 python sync_to_github.py`（冪等・キャッシュで成功分はスキップ→残51本を回収） ②`python <scratch>/push_remote.py --apply`（**リモート最新版を取得→文字列置換→PUT**＝ローカルを送らないのでクラウド先行分の巻き戻し無し。対象110本のリストは `api_targets.json`） ③`mw trigger update-market-news.yml`（6コア＋track-record/political-feed/youtube-summary を再生成）。※scratchのスクリプトは揮発するので、再開時は本文書の仕様から作り直すか `apply_anatomy_link.py` を手本にする。
