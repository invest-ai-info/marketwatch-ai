...
- **🆕 複利シミュレーター公開（7/4・オーナー発案）**：`guide-compound-sim.html`＝入力4つ（初期額/月額/年利/期間）→資産推移SVGグラフ＋年次表＋単利比較＋課税概算トグル（20.315%・NISA注記）。全て端末内JS・外部送信なし。**計算式＝金融庁つみたてシミュレーター2025年9月改修後と同じ幾何月利 (1+年利)^(1/12)−1**（監査Opusが「旧式=年利÷12＋『金融庁と同じ』表記は改修で不正確＝優良誤認リスク」を発見→式ごと新式化・権威付け文言除去）。単利は年利÷12で分離。ゲート＝プレビュー実走でPython参照値と全ケース一致→コンプラ🟡→修正→独立白→publish→HTTP200。カテゴリ「💰投資の基礎知識」。今後の単複利記事（autodraftキュー⑬）から内部リンク推奨。**→常設導線2本を追加（同日・ライブ確認済み）**：①index＝`generate_market_news.py` テンプレの更新履歴直下に🧮固定バナー（生成テンプレ埋め込み＝毎回再生成でも消えない） ②guides.html最上段に「🧮 計算ツール（常設）」カテゴリ新設（速報リストのカードは新着流れ用にそのまま）。ツールが増えたらこのカテゴリが受け皿。
