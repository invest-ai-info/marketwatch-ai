...
### 🔜 次セッションで最初に確認（在flight・2026-07-21向け）
- **⓪-明日の計画（7/20夜時点・オーナー合意済み）**: ①**明朝06:10確認=trackerに`state_*`5本出現**（STATE_2026_07_20ブートストラップ・edge3+gate2）＋既存コンボ/strike正常 ②**特別記事「指標の組み合わせ大全——死んだ指標は場所で蘇り、弱い逆張りは静かに死ぬ」執筆・公開**（素材=`_state_sweep_bt.json`+失敗転換スタディ+7/19コンボ定点。いつものゲート=Opusコンプラ白→publish） ③**UX残り=記事約75本へ「↑上に戻る」一括適用**（fix_mobile_overflow.py型の冪等ツール新設） ④新種の設計=**出来高/人気×シグナル成否**（オーナー発案・queueバックログ登録済み＝発火時相対出来高のBT遡及+news_count層別を事前登録してから）。
