...
> ## ⚖️ (g) 法務棚卸し＝黒13件修正・完了（8/1・全文は SESSION_ARCHIVE）／💾 (h) キオクシア記事＋`_gen_stock_panel.py`（8/1・全文は SESSION_ARCHIVE）
> 持ち越す要点だけ: ① **記事末尾の「対応戦略」テンプレが黒の製造機**（11/13がここ）＝「読者に何をさせるかで締める」書き方をしない。
...
> - 🆕 **生成スクリプトの CSS は f-string ＝波括弧が二重**。素の形だけで名指しすると**生成ページだけ素通り**する
>   （記事は全部塗れているのに6コアのナビだけ旧青、という気づきにくい壊れ方）。対策は `fstring_form()`＋テスト⑱⑲＝**コード側で解決済み**
...
> - 🆕 **回線輻輳下の押し込み方**＝1本ずつは抜けられない。tree API で束ねる（**進捗はリポジトリ自身が持つ**＝落ちても次回 tree 取得で残りが減る）
>   ＝`sync_to_github.py --batch` として**コード側で解決済み**
>
> ### 未完の作業
> | # | 内容 |
> |---|---|
