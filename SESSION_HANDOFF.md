# 🔖 セッション引き継ぎ（最終更新: 2026-08-14 21:20）

## 🚩 次のセッションは、ここから（状態はすべて 2026-08-14 21:20 JST の実測）

### ① 最初にやること — 詐欺#2がローカルで止まっている（所要5分）

`guide-scam-romance-invest.html`（58KB・本日16:30作成）が **8ステップの②〜⑤未実行のまま**。実測＝決定論ゲート **GREEN**（`check_guide_draft.py` exit 0）／ライブ **404**／**main に無し**（Contents API `?ref=main`）／`SYNC_FILES` 未登録／`guides.html` カード無し／更新履歴なし。
一方 **格言#34（損切りは早く利は伸ばせ）は本日の routine が公開済み**（ライブ200・main 有り）＝止まっているのは詐欺レーンだけ。

```
python mw.py publish --file guide-scam-romance-invest.html --category "投資詐欺から身を守る" --emoji 💔 \
    --card-title "SNS型ロマンス詐欺——恋愛感情×投資誘導の手口を段階分解" \
    --desc "2026年上半期246.6億円・2,509件（警察庁）。恋愛感情を育てた後に投資へ誘導する手口の段階分解と、お金の話が出た瞬間の確認手順・相談窓口。"
```
（`--dry-run` で確認 → sync → workflow → ライブ確認。`--category` は第1号と同名＝`guides.html` に実在するので新セクションは作られない）

- 🔴 `python mw.py check` が**この1件だけで ❌ NG（error）**＝**公開するまで他の sync も止まる**（`SYNC_FILES に未登録`＋`guides.html にカードが無い`。リンターが独立に同じ結論を出している）
- 🩹 ついでに `python apply_back_to_top.py`＝**詐欺#1・#2 とも「↑上に戻る」ボタンが無い**（warning。#1 は既にライブ）
- ⏰ **期限＝明日 8/15 11:12 の `scam-daily-auto`**。`SCAM_GUIDE.md` のキュー行 #2 に ✅公開済 が付いていないので、**放置するとクラウドが #2 をもう一度書く**（重複／スラッグ衝突）。出さないと決めたなら**先にキュー行へ理由を書く**。
- ⚠️ 公開前に決めること＝**カードの置き場所**。ライブ実測で第1号は「🧮 計算ツール（常設）」節の先頭に入っている（`guides.html` に詐欺シリーズの専用セクションが無いため）。第2号も同じ場所に並ぶ。**専用節を作るなら公開前が一番安い**。

### ② 今夜〜明朝に見るもの

- 直近 workflow 8件すべて success（21:12 JST 実測）＝**今は緑**。今夜残るのは 22:00 political-digest だけ
- 土曜は routine が集中する日：05:30 autodraft／06:10 signal-lab（#068）／08:40 autodraft-publish／**09:30 automation-health**／**10:00 site-qa-lint**／10:12 格言#35／**11:00 book-watch**／11:12 詐欺／**12:00 weekly-trade-review**
- 見るのは1つでいい＝**09:30 の automation-health**。詐欺#2 を公開したなら「カードが消えていないか（§③）」もここに出る

### ③ オーナーの手が要るもの

| 件 | 状態 |
|---|---|
| 🔴 **弁護士相談の予約** | 資料 `_lawyer_consult_2026.md` が本日 21:02 に完成（11項目・最重要＝④「不特定多数向け・随時閲覧可能な媒体」除外の射程）。**あとは予約するだけ**。期日ゲート＝メアド100件 or 2026-10-31 で、グレー全域が自動的に「白のみで設計」へ格下げ |
| **Issue #4 の始末** | 8/12 の automation-health Issue が開いたまま。中身（#065/#067 エスカレ）は 8/12 に公開して解決済み＝**閉じてよいかの確認だけ**。#3 は 6/21 の古い残骸 |
| キュー残 | 格言 **#35〜39 の5本**＝枯渇 ≒ **8/20**／詐欺 **#3〜10 の8本**。補充は枯渇の2〜3日前に |
| 唯一の未完タスク | ブランドカラー **Phase 2**（本文リンク＝藍(明)＋下線・承認済み・実測306本895箇所。`--phase 2` は実装済み） |
| AdSense | 再却下→記事蓄積後に再申請（リモートの guide 記事は現在 **323本**） |
