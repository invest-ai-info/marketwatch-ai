...
## 🔜 次にやる候補：④ 記事の「中身」レビュー強化（読みやすさ・トーン・事実の明快さ）

機械的な不具合（SVG重なり/はみ出し・免責漏れ・数値と claim の不一致 など）は決定論チェックで自動ブロック済み。**残るは内容品質の底上げ＝LLMレビュー軸**（オーナー要望「記事の精度・クオリティを上げたい」の続き）。
- **既存資産**：`compliance-reviewer`(Opus)＋自動公開Opusゲート＋`content-writer`＋`seo-ux-strategist`（CLAUDE.md「サイト運営チーム」）。
- **設計案**：公開ゲートに「品質/読みやすさ」観点を1レーン追加。観点例＝①リードが結論を30秒で伝えるか ②専門用語に初出説明があるか ③主張に根拠/数値が紐づくか ④断定でなく中立トーンか ⑤見出しと中身が一致しているか。compliance（黒/グレー/白）とは別の「品質スコア」にするか、Opusゲートのプロンプトに統合するかは要検討。
- **進め方**：まず1記事で観点リストを試行→効果確認→ signal-lab / news-daily の自動公開ゲートへ組み込み（過剰にせず、機械チェックで拾えない"内容"だけをLLMに任せる）。⚠️ signal_lab_verify.py（固定オラクル）には触れず、別レイヤ（プロンプト/別スクリプト）で。
- 関連：[[feedback_rules_as_code]]（機械的=コード、内容=レビュー の住み分け）。
