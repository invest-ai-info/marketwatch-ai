...
- **⓪-図解カタログ 7/24夜✅（オーナー依頼「シグナル発火を図解で・今後の日誌もチャートで」）**: ①**特別記事公開=`guide-signal-anatomy.html`**（「AIはどんな瞬間にシグナルを出すのか——発火条件の図解カタログ」🧪・チャート風SVG3枚=逆張り3種/順張り3種/通知パイプライン・発火条件はエンジンコードから正確に転記・独立Opus監査**🟢白=黒ゼロ修正なし**・グレー2件は既存弁護士アジェンダ①②へ実例添付のみ・ライブ200/guidesカード/SVGはみ出し0/ダーク両対応 確認済み） ②**signal-lab-daily routineプロンプト更新**=SVGのうち1枚「チャート風図解」を必須化（手本=anatomy図1・図2/概念図注記必須/状況次元(env・news等)は背景帯で表現/定点観測日は省略可）＋関連カードにanatomy必須＋**filterキー列挙を16キー現行化（成果戦略・残タスク(iv)消化）**。**→7/25朝06:10の#050が初適用=チャート図解が入るか要確認** ③**過去日誌46本へ導線注入完了**=`apply_anatomy_link.py`新設（冪等・meta-line直後にinfo-box・SYNC入り）。⚠️日誌はクラウド公開レーン＝ローカルSYNC_FILES外→**Contents APIで一回限り修復push46/46**（7/21ナビ修復と同じ定石）・ライブ4本抜き取り確認済み。**残宿題=過去日誌への個別チャート図解の後付け**（46本一括は重い→重要回=昇格/降格回から数本ずつ or /loopレシピ化＝進め方はオーナー相談）。
