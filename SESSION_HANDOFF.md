...
- **⓪-I 🚨guides.htmlカード消失を検知（7/9 19:55・未対応）**: `guide-news-2026-07-06-hormuz-oil-glut.html` の記事HTMLはrootにあるがGitHub側guides.htmlにカードが無い（カバレッジ番人③が検知・他news23件は正常）。guides.html直近編集=7/9 17:48 news-daily公開。**いつ消えたかをguides.html履歴の二分探索で特定→巻き戻しなら復元、意図的取り下げ（7/6は重複公開事故対応日）なら除外リスト化を相談**。放置すると毎朝automation-healthがIssueを立てる。
