# 🔄 セッション引継ぎ票（最終更新: 2026-05-28 夜 — AMD 第 5 弾公開 + サイト運営チーム 3 人 動作テスト完了）
...
### サイト運営チーム 3 人 動作テスト + AMD 第 5 弾公開

**動作テスト結果**:
- AMD 個別銘柄解説 第 5 弾の企画案を 3 人並列起動（content-writer / seo-ux-strategist / compliance-reviewer）
- 並列レビュー → 統合企画 → 本文執筆 → compliance 事後監査 🟢 公開推奨 → 8 ステップ実装 → push
- 所要時間: 約 3 時間（うち 3 人並列レビュー部分は約 90 秒）

**学び（次回への申し送り）**:
- subagent は **プロジェクト直下の cwd でセッション起動しないと `.claude/agents/` を認識しない**
- 別 cwd から起動した場合は `.claude/agents/*.md` を Read で読み込んで general-purpose に inline 注入する代替策が機能した
- 推奨運用: サイト系作業は必ず `C:\Users\info0\OneDrive\デスクトップ\新しいフォルダー` を cwd にしてセッション起動

**AMD 第 5 弾の特徴**:
- ファイル: `guide-amd-2026-05.html`（35.8 KB、約 9,000 字、読了 14 分）
- 内容: Q1 2026 決算（売上 $10.3B / DC $5.8B、Q2 ガイド $11.2B）+ MI300X→MI400 ロードマップ + ZT Systems 統合 + 5 シナリオ（強気 2 / 弱気 2 / 中立 1、個社競争力軸）+ 日本人投資家への 5 つの示唆
- NVIDIA 記事との差別化: マクロ崩壊軸 → 個社競争力軸、CPU+GPU 両輪、ROCm vs CUDA 論点を独立扱い
- 法務予備チェック → 黒 6 項目全回避、グレー 6 項目全充足、必須実装完備

**副次的改善**:
- guides.html の Kioxia カテゴリ移動（投資指標解説 → 速報・タイムリー記事、badge も「個別銘柄解説」に統一）
- 半導体 3 部作（Kioxia / NVIDIA / AMD）が同一カテゴリに揃った

**ライブ反映**:
- commit `0985a2f`（AMD HTML）+ `31f1a64`（guides.html）push 済
- ユーザー手動で Update Market News workflow を trigger 予定

### 🧭 2026-05-28 夜の方針確認（次回以降の指針）

**ユーザー方針**: 「逆張りは当面保留。Web サイトのクオリティ強化に注力。データが揃ったら再びシグナルメール改善へ」

- **逆張りバックテスト結論**: 銘柄一律ではダメ、USDJPY のみ PF 1.41 の手応えあり → **追加検証は当面保留**（`backtest_range_reversal.py` はローカルに残置、SYNC_FILES 未追加）
- **次回セッション以降の優先順**（サイトクオリティ強化系）:
  1. 既存 guide-*.html 33 ファイルへのフッターディスクレイマー一括追加（B2 残作業）
  2. about.html / privacy.html / contact.html のナビバー 9 ボタン統一（D3）
  3. about.html のデータソース透明化（D2）
  4. 個別銘柄解説 第 6 弾（TSMC / SBI から選定）
  5. note 開設・第 1 記事執筆
- **シグナルメール改善の再開タイミング**: B2 信頼度（目標 6/15）・C1 週次（目標 6/8）・C3 マンスリー（目標 6/2）等のデータ蓄積完了後
...
13. 個別銘柄解説 第 6 弾（TSMC / SBI から選定、AMD は 5/28 完了）
